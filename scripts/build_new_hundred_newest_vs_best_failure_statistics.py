#!/usr/bin/env python3
"""Hundred-case failure statistics: newest strict-phased method vs best exact-loss comparator.
...
Comparator and newest method are resolved from current canonical status artifacts.
"""
...
STRICT_PHASED_DEFAULT_DOC = (
    REPO_ROOT / "docs/BROADER_STRICT_PHASED_DEFAULT_DECISION_EVAL_20260421T023506Z.md"
)
...
def _resolve_newest_method() -> tuple[str, dict[str, Any]]:
    fallback = "broad_diversity_aggregation_strong_v1_anti_collapse_answer_group_refinement_repeat_expansion_fine_incumbent_guard_tuned_v1_hard_early_root_depth2_coverage_forced_v1__deterministic_output_layer_repair_v1"
    method = fallback
    strict_gate1 = None
    strict_gate2 = None
    if STRICT_PHASED_DEFAULT_DOC.exists():
        txt = STRICT_PHASED_DEFAULT_DOC.read_text(encoding="utf-8")
        for line in txt.splitlines():
            if line.startswith("- `strict_gate1`:"):
                strict_gate1 = line.split("`")[-2]
            if line.startswith("- `strict_gate2`:"):
                strict_gate2 = line.split("`")[-2]
        m = re.search(r"Proposed default promoted model: \*\*([a-zA-Z0-9_]+)\*\* \(`([^`]+)`\)", txt)
        if m:
            method = m.group(2)
    return (
        method,
        {
            "selection_rule": "Use newest strict-phased broad-default recommendation when available; otherwise strongest strict candidate from newest status docs",
            ...
            "strict_gate1_method": strict_gate1,
            "strict_gate2_method": strict_gate2,
            "resolved_method": method,
            "ambiguity_resolution_between_gate1_gate2": (
                "strict_gate1 and strict_gate2 were both candidate gates; broad strict-phased default decision doc recommends strict_f2 as broad default, so strict_f2 is selected for newest-method exact-loss analysis."
            ),
        },
    )


def _resolve_best_comparator() -> tuple[str, dict[str, Any]]:
    hinted = "reasoning_beam2"
    if EXACT_LOSS_COMPARATOR_DOC.exists():
        txt = EXACT_LOSS_COMPARATOR_DOC.read_text(encoding="utf-8")
        m = re.search(r"fresh (?:exact )?loss-set.*was `([^`]+)`", txt, flags=re.IGNORECASE | re.DOTALL)
        if m:
            hinted = m.group(1)
    return (
        hinted,
        {
            "selection_rule": "Use latest canonical exact-loss/fresh-loss surface direct adversary from status docs",
            "exact_loss_evidence_doc": str(EXACT_LOSS_COMPARATOR_DOC.relative_to(REPO_ROOT)),
            "broad_bundle_context": "Older/broader light or matched bundle leaders can differ (e.g., self_consistency_3 in older pass); exact-loss comparator is resolved separately.",
            "resolved_best_comparator": hinted,
        },
    )
...
def main() -> None:
    ...
    out_dir = REPO_ROOT / f"outputs/new_hundred_newest_vs_best_failure_statistics_{ts}"
    out_dir.mkdir(parents=True, exist_ok=True)

    current_method, newest_resolution = _resolve_newest_method()
    best_method, comparator_resolution = _resolve_best_comparator()
    ...
    manifest = {
        "artifact_family": "new_hundred_newest_vs_best_failure_statistics",
        "created_at_utc": now.isoformat(),
    ...
    doc_path = REPO_ROOT / f"docs/NEW_HUNDRED_NEWEST_VS_BEST_FAILURE_STATISTICS_{ts}.md"
    ...
    lines: list[str] = []
    lines.append("# New hundred-case failure statistics: newest strict-phased method vs best exact-loss comparator")
    lines.append("")
    ...
    lines.append(f"- **Selected newest method**: `{current_method}`")
    ...
    lines.append(
        f"1. **Eligibility**: On that grid, keep rows where the newest selected method is incorrect and `{best_method}` is correct."
    )
    lines.append(
