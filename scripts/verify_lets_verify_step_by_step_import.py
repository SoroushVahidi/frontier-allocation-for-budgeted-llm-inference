#!/usr/bin/env python3
"""Verify Let's Verify Step by Step adjacent import package conservatively."""
...
REQUIRED_METADATA_FIELDS = [
    ...
    "dataset.name",
    "dataset.split",
    "evaluation.budget.pass_at_k_values",
    "evaluation.budget.verifier_score_threshold",
    "evaluation.budget.compute_budget_note",
    "models.generator_model_family",
...
def _check_official_repo_path(official_repo_path: Path | None, contract_config: Path) -> dict[str, Any]:
    ...
    try:
        ...
        required_layout = [str(x) for x in contract.get("model_and_path_requirements", {}).get("official_repo_layout_required", [])]
...
def verify_lets_verify_step_by_step_import(
    *,
    requested_path: Path,
    ...
    for idx, row in enumerate(rows):
        ...
        if mode != "prm800k_adjacent_import":
        ...
        for int_col in ["pass_at_k", "num_examples"]:
        ...
        threshold_raw = str(row.get("verifier_score_threshold", "")).strip()
        try:
            float(threshold_raw)
        except ValueError:
            issues.append(f"non_numeric_verifier_score_threshold_row_{idx}")
        ...
        normalized_rows.append(
            {
                "mode": "prm800k_adjacent_import",
                "source": "lets_verify_step_by_step_import_verified",
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Verify Let's Verify Step by Step adjacent import package")
...
def main() -> None:
    ...
    report = verify_lets_verify_step_by_step_import(
        ...
        contract_config=Path(args.contract_config).resolve(),
