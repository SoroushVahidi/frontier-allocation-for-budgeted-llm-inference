#!/usr/bin/env python3
"""Verify TALE MODE B official/full import package against strict contract."""
...
REQUIRED_METADATA_FIELDS = [
    ...
    "tale_variant.kind",
    "tale_variant.name",
    "tale_variant.includes_post_training",
    "model.checkpoint",
    "dataset.name",
    "dataset.split",
    "prompt.template",
    ...
    "provenance.version_or_commit_if_available",
]
...
MODE_A_METHOD_MARKERS = {"adaptive_min_expand_1", "external_tale_prompt_budgeting"}
ALLOWED_SOURCE_TYPES = {"official", "author-produced", "imported"}
ALLOWED_VARIANT_KINDS = {"tale_inference_budgeting", "tale_pt", "official_tale_other"}
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Verify TALE MODE B import package")
...
def main() -> None:
    ...
    metadata_variant = {"kind": "", "name": "", "includes_post_training": ""}
    if metadata:
        ...
        metadata_variant = {
            "kind": str(_get_nested(metadata, "tale_variant.kind") or ""),
            "name": str(_get_nested(metadata, "tale_variant.name") or ""),
            "includes_post_training": str(_get_nested(metadata, "tale_variant.includes_post_training") or ""),
        }
        if metadata_variant["kind"] not in ALLOWED_VARIANT_KINDS:
            issues.append("invalid_tale_variant_kind")
        ...
        if dataset_name != args.expected_dataset:
            issues.append(f"dataset_mismatch: expected={args.expected_dataset}, observed={dataset_name}")
    ...
    observed_variant_triples: set[tuple[str, str, str]] = set()
    ...
    for idx, row in enumerate(rows):
        ...
        variant_kind = str(row.get("tale_variant_kind", "")).strip()
        variant_name = str(row.get("tale_variant_name", "")).strip()
        includes_pt = str(row.get("includes_post_training", "")).strip().lower()

        if mode == "prompt_budgeting_inference_only":
        ...
        if variant_kind not in ALLOWED_VARIANT_KINDS:
            issues.append(f"invalid_tale_variant_kind_row_{idx}")
        ...
        observed_variant_triples.add((variant_kind, variant_name, includes_pt))
        ...
        normalized_rows.append(
            {
                ...
                "tale_variant_kind": variant_kind,
                "tale_variant_name": variant_name,
                "includes_post_training": includes_pt,
                "dataset": dataset,
                "dataset_split": split,
                ...
                "version_or_commit": row.get("version_or_commit", ""),
            }
        )

    if len(observed_variant_triples) > 1:
        issues.append("mixed_tale_variants_detected")

    if metadata and observed_variant_triples:
        observed = sorted(observed_variant_triples)[0]
        meta_kind = metadata_variant["kind"]
        meta_name = metadata_variant["name"]
        meta_pt = str(metadata_variant["includes_post_training"]).lower()
        if (meta_kind, meta_name, meta_pt) != observed:
            issues.append("metadata_row_variant_mismatch")
    ...
    verification = {
        ...
        "observed": {
            ...
            "observed_variant_triples": sorted([list(x) for x in observed_variant_triples]),
        },
