...
CANONICAL_REFERENCE_SURFACE = "canonical_full_method_ranking_20260421T212948Z"
DATASETS = ["openai/gsm8k", "HuggingFaceH4/MATH-500", "HuggingFaceH4/aime_2024", "Idavidrein/gpqa"]
BUDGETS = [4, 6, 8]
...
def _load_twenty_module() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("twenty_exact_bundle_for_science_augmented", path)
...
def _run_method(method_public: str, method_runtime: str, dataset: str, seed: int, budget: int, example: Any) -> dict[str, Any]:
    run_seed = _stable_seed("science_augmented_matched", method_public, dataset, seed, budget, example.example_id)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run science-augmented matched-surface decision rerun.")
    ...
    out_dir = REPO_ROOT / "outputs" / f"science_augmented_matched_surface_{args.run_id}"
    ...
    runnable = [(p, r) for p, r in METHOD_SPECS if r in probe]
    excluded = [
        {"method": p, "runtime_key": r, "reason": "runtime_key_not_available"}
        for p, r in METHOD_SPECS
        if r not in probe
    ]
    ...
    method_rank = []
    for rank, rec in enumerate(comparison, start=1):
        method_rank.append(
            {
                "rank": rank,
                "method": rec["method"],
                "method_class": rec["method_class"],
                "mean_accuracy": rec["mean_accuracy"],
                "n_cases": rec["n_cases"],
            }
        )

    pair_specs = [("strict_f3", "strict_gate1_cap_k6"), ("strict_f3", "strict_f2"), ("strict_gate1_cap_k6", "strict_f2")]
    pair_rows = []
    for b in budgets:
        slice_rows = {str(r["method"]): r for r in per_budget if int(r["budget"]) == b}
        for a, c in pair_specs:
    ...
    _write_csv(out_dir / "head_to_head_summary.csv", pair_rows)
    _write_csv(out_dir / "method_ranking_summary.csv", method_rank)
    ...
    f3 = _acc("strict_f3")
    gate = _acc("strict_gate1_cap_k6")
    f2 = _acc("strict_f2")
    top_internal = max([
        ("strict_f3", f3),
        ("strict_gate1_cap_k6", gate),
        ("strict_f2", f2),
    ], key=lambda x: x[1])[0]
    gpqa_rows = [r for r in per_dataset if str(r["dataset"]) == "Idavidrein/gpqa"]
    non_gpqa_rows = [r for r in per_dataset if str(r["dataset"]) != "Idavidrein/gpqa"]

    def _slice_mean(method: str, rows_in: list[dict[str, Any]]) -> float:
        vals = [float(r["mean_accuracy"]) for r in rows_in if str(r["method"]) == method]
        return _mean(vals)

    f3_gpqa = _slice_mean("strict_f3", gpqa_rows)
    gate_gpqa = _slice_mean("strict_gate1_cap_k6", gpqa_rows)
    f2_gpqa = _slice_mean("strict_f2", gpqa_rows)
    f3_non = _slice_mean("strict_f3", non_gpqa_rows)
    gate_non = _slice_mean("strict_gate1_cap_k6", non_gpqa_rows)
    f2_non = _slice_mean("strict_f2", non_gpqa_rows)

    mechanism_note = "mixed_or_not_assessed"
    f3_gate = [r for r in pair_rows if r["method_a"] == "strict_f3" and r["method_b"] == "strict_gate1_cap_k6"]
    if f3_gate:
        abs_mag = sum(abs(float(r["delta_absent_from_tree_rate_a_minus_b"])) for r in f3_gate)
        pns_mag = sum(abs(float(r["delta_present_not_selected_rate_a_minus_b"])) for r in f3_gate)
        olm_mag = sum(abs(float(r["delta_output_layer_mismatch_rate_a_minus_b"])) for r in f3_gate)
        if (abs_mag + pns_mag) > olm_mag:
            mechanism_note = "tree_entry_plus_selection_dominant"

    lines = [
        "# Science-augmented matched-surface decision note",
        "",
        "This run is a decision-stability stress test and does not overwrite canonical manuscript artifacts.",
        f"- canonical reference surface: {CANONICAL_REFERENCE_SURFACE}",
        f"- datasets: {datasets}",
        f"- budgets: {budgets}",
        f"- seeds: {seeds}",
        f"- subset_size: {args.subset_size}",
        f"- methods_runnable: {[m for m, _ in runnable]}",
        "",
        "## Direct decision answers",
        f"1. Does strict_f3 remain best internal on science-augmented matched surface? {'yes' if top_internal == 'strict_f3' else 'no'} (top_internal={top_internal}, mean_acc: f3={f3:.4f}, gate={gate:.4f}, f2={f2:.4f}).",
        f"2. Does strict_gate1_cap_k6 become stronger? {'yes_relative_to_strict_f3' if gate > f3 else 'not_on_overall_surface'} (overall gap gate-f3={gate-f3:+.4f}; gpqa slice gate-f3={gate_gpqa-f3_gpqa:+.4f}; non-gpqa slice gate-f3={gate_non-f3_non:+.4f}).",
        f"3. Does strict_f2 emerge as broader internal candidate? {'yes' if f2 >= max(f3, gate) else 'mixed'} (overall gap f2-f3={f2-f3:+.4f}, f2-gate={f2-gate:+.4f}; gpqa slice f2={f2_gpqa:.4f}).",
        f"4. Does adding GPQA materially change manuscript-facing claim strength? {'yes_weakened' if top_internal != 'strict_f3' else 'partially_no'} (non-gpqa avg: f3={f3_non:.4f}, gate={gate_non:.4f}, f2={f2_non:.4f}; with gpqa overall top={top_internal}).",
        "5. Should manuscript positioning change now? keep unchanged for now; treat as stress-test evidence and consider reconsideration only with additional confirming reruns.",
        "",
        f"Mechanism summary (compact): {mechanism_note}.",
    ]
    (out_dir / "conservative_interpretation_note.md").write_text("\n".join(lines) + "\n", encoding="utf-8")

    manifest = {
        "artifact_family": "science_augmented_matched_surface",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "script": "scripts/run_science_augmented_matched_surface.py",
        "output_dir": str(out_dir.relative_to(REPO_ROOT)),
        "contract": {
            "canonical_reference_surface": CANONICAL_REFERENCE_SURFACE,
            "datasets": datasets,
            "budgets": budgets,
            "seeds": seeds,
