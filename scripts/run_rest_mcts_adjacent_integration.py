#!/usr/bin/env python3
"""Run stable ReST-MCTS adjacent integration contract lane.

This lane intentionally stays conservative:
- validates a contract-defined adjacent import package,
- verifies official-source references and optional upstream repo layout,
- emits a canonical artifact family for paper-safe reporting.
"""
...
from verify_rest_mcts_import import verify_rest_mcts_import

REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CONTRACT = REPO_ROOT / "configs" / "rest_mcts_adjacent_comparison_contract_v2.json"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "outputs" / "rest_mcts_adjacent_integration"
DEFAULT_OFFICIAL_REPO_PATH = Path("/tmp/rest_mcts_upstream/ReST-MCTS")
...
def _try_clone_if_missing(path: Path) -> dict[str, Any]:
    ...
    cmd = ["git", "clone", "--depth", "1", "https://github.com/THUDM/ReST-MCTS.git", str(path)]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run stable ReST-MCTS adjacent integration lane")
...
def main() -> None:
    ...
    commands: list[str] = [
        f"python scripts/run_rest_mcts_adjacent_integration.py --contract-config {contract_path.relative_to(REPO_ROOT)}"
    ]
    ...
    comparison_readiness = {
        ...
        "unsafe_claims": contract.get("not_allowed_claims", []),
    }

    status_payload = {
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "baseline_key": "rest_mcts",
        "canonical_title": contract.get("canonical_title", "ReST-MCTS*"),
        "classification": classification,
    ...
    summary_md = "\n".join(
        [
            "# ReST-MCTS adjacent integration summary",
            "",
            f"- Run ID: `{run_id}`",
            ...
            "## Scope guardrail",
            "",
            "- This run is **adjacent-only** and does not claim full faithful in-repo ReST-MCTS reproduction.",
    ...
    manifest = {
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "run_id": run_id,
        "script": "scripts/run_rest_mcts_adjacent_integration.py",
        "output_dir": str(run_dir.relative_to(REPO_ROOT)),
        "commands": commands,
