...
MATH_DATASETS = ["openai/gsm8k", "HuggingFaceH4/MATH-500", "HuggingFaceH4/aime_2024"]
NON_MATH_DATASETS = ["google-deepmind/natural-plan", "Idavidrein/gpqa", "TIGER-Lab/MMLU-Pro"]
...
def _load_gpqa_examples(subset_size: int, seed: int) -> list[PilotExample]:
    ...
    for i, row in enumerate(picked):
        ...
        prompt = "\n".join([q, "", "Choose one option and answer with a single letter (A/B/C/D).", *[f"{labels[j]}. {options[j]}" for j in range(4)]])
        out.append(PilotExample(example_id=f"gpqa_diamond_{seed}_{i}", question=prompt, answer=labels[options.index(correct)] if correct in options else "A"))
    return out
...
def _perm_pvalue(diffs: list[float], n_perm: int, seed: int) -> float:
    ...
    for _ in range(n_perm):
        stat = abs(_mean([d * (1.0 if rng.random() < 0.5 else -1.0) for d in diffs]))
        if stat >= obs - 1e-12:
            geq += 1
...
def _pairwise_tests(rows: list[dict[str, Any]], comparisons: list[tuple[str, str]], n_boot: int = 3000, n_perm: int = 3000) -> list[dict[str, Any]]:
    idx = {(str(r["dataset"]), int(r["budget"]), int(r["seed"]), str(r["example_id"]), str(r["method"])): int(r["is_correct"]) for r in rows}
    out = []
...
def _method_specs(observed: Any, budget: int, rng: random.Random) -> dict[str, Any]:
...
def _run_method(method_public: str, method_runtime: str, dataset: str, seed: int, budget: int, ex: PilotExample) -> dict[str, Any]:
    run_seed = _stable_seed("tot_matched_budget", method_public, dataset, seed, budget, ex.example_id)
    rng = random.Random(run_seed)
    observed = TW.ObservedGenerator(SimulatedBranchGenerator(rng=rng, max_depth=7, finish_prob_base=0.16, answer_noise=0.12))
    specs = _method_specs(observed, budget, rng)
    ...
    extraction_mismatch = bool(rep.get("chosen_final_node_answer_canonical") != rep.get("extracted_final_answer_canonical") or rep.get("extracted_final_answer_canonical") != rep.get("surfaced_final_answer_canonical"))
    if not gold_in_tree:
    ...
    return {
        ...
        "budget_respected": int(int(result.actions_used) <= int(budget)),
    }


def _select_datasets(run_non_math: bool) -> tuple[list[str], list[dict[str, Any]]]:
    datasets = list(MATH_DATASETS)
    infeasible: list[dict[str, Any]] = []
    if not run_non_math:
        return datasets, infeasible

    access = check_git_dataset_access("google-deepmind/natural-plan")
    if not bool(access.get("ok")) and str(access.get("clone_command", "")).strip():
        try:
            subprocess.run(str(access["clone_command"]).split(), check=True, cwd=REPO_ROOT, capture_output=True, text=True)
        except Exception:
            pass
        access = check_git_dataset_access("google-deepmind/natural-plan")
    ...
    if bool(access.get("ok")):
        ...
        infeasible.append({"dataset": "google-deepmind/natural-plan", "reason": "unavailable"})

    try:
        _ = _load_gpqa_examples(subset_size=2, seed=11)
        datasets.append("Idavidrein/gpqa")
    except Exception as e:
        infeasible.append({"dataset": "Idavidrein/gpqa", "reason": f"load_failed:{type(e).__name__}"})

    datasets.append("TIGER-Lab/MMLU-Pro")
    return datasets, infeasible


def _load_dataset_examples(dataset: str, subset_size: int, seed: int, natural_plan_task: str) -> list[PilotExample]:
    if dataset == "google-deepmind/natural-plan":
        return _load_natural_plan_examples(subset_size=subset_size, seed=seed, task_name=natural_plan_task)
    if dataset == "Idavidrein/gpqa":
        return _load_gpqa_examples(subset_size=subset_size, seed=seed)
    return load_pilot_examples(dataset, subset_size=subset_size, seed=seed)


def main() -> None:
    p = argparse.ArgumentParser(description="Run matched-budget ToT baseline comparison bundle.")
    ...
    datasets_to_run, infeasible = _select_datasets(run_non_math=not bool(args.math_only))

    # method availability probe
    probe_rng = random.Random(0)
    probe_specs = _method_specs(SimulatedBranchGenerator(rng=probe_rng, max_depth=7, finish_prob_base=0.16, answer_noise=0.12), max(budgets), probe_rng)
    runnable_methods: list[tuple[str, str]] = []
    for m in METHODS_REQUESTED:
        runtime = PUBLIC_TO_RUNTIME.get(m, m)
        if runtime in probe_specs:
            runnable_methods.append((m, runtime))
    ...
    for dataset in datasets_to_run:
        for seed in seeds:
            examples = _load_dataset_examples(dataset, subset_size=int(args.subset_size), seed=seed, natural_plan_task=str(args.natural_plan_task))
            for budget in budgets:
                for ex in examples:
                    for method_public, method_runtime in runnable_methods:
                        per_case.append(_run_method(method_public, method_runtime, dataset, seed, budget, ex))
    ...
    frontier_candidates = [m for m in ["strict_f3_anti_collapse_weak_v1", "strict_f3", "strict_gate1_cap_k6"] if any(s["method"] == m for s in main_summary)]
    best_frontier = sorted(
        [s for s in main_summary if s["method"] in frontier_candidates],
        key=lambda r: (-float(r["mean_accuracy"]), float(r["avg_actions"]), str(r["method"]),),
    )[0]["method"] if frontier_candidates else "strict_f3"

    comparisons = [
        (best_frontier, "tot_bfs_matched_budget"),
        (best_frontier, "tot_beam_matched_budget"),
        (best_frontier, "tot_dfs_matched_budget"),
        (best_frontier, "self_consistency_5"),
        ("tot_bfs_matched_budget", "self_consistency_5"),
        ("tot_bfs_matched_budget", "external_l1_max"),
        ("strict_f3_anti_collapse_weak_v1", "strict_f3"),
    ]
    pairwise = _pairwise_tests(per_case, comparisons=comparisons)
    ...
    manifest = {
        ...
        "best_frontier_variant": best_frontier,
        "tot_adapter_note": "Matched-budget adapter baseline; not an official Tree-of-Thoughts reproduction.",
        "baseline_scope_note": "ReST-MCTS* and Graph-of-Thoughts excluded in this adapter due to reward-model/self-training and graph-operation requirements.",
    }
    (out_dir / "manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")

    summary_lines = [
        "# ToT matched-budget baseline summary",
        "",
        f"- Datasets ran: {datasets_to_run}",
        f"- Budgets: {budgets}; seeds: {seeds}; subset size: {int(args.subset_size)}",
        f"- Best frontier variant on this run: {best_frontier}",
        "- ToT variants are matched-budget adapters (`tot_bfs_matched_budget`, `tot_beam_matched_budget`, `tot_dfs_matched_budget`), not official reproductions.",
        "- Budget accounting rule: one action equals one branch expansion call; scoring is folded into that expansion with no extra verifier calls.",
        "- ReST-MCTS* not implemented in this baseline because it requires process-reward-guided self-training/learned reward machinery.",
        "- Graph of Thoughts not implemented here because graph merge/distill/feedback operations require a stricter adapter and may drift from local frontier allocation constraints.",
        "",
        "## Caution",
        "- This bundle does not support universal dominance claims.",
        "- Treat conclusions as matched-surface evidence under listed datasets/budgets/seeds.",
    ]
