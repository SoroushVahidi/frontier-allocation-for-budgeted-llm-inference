...
def main() -> None:
    ...
    p.add_argument("--input", default=str(PLOT_DATA_DIR / "oracle_gap_curves.csv"))
    ...
    for method in grouped_methods(rows):
        ...
        ax.plot([int(r["budget"]) for r in mrows], [float(r["oracle_gap"]) for r in mrows], marker="o", label=method)

    style_ax(ax, "Oracle Gap by Budget (GSM8K)", "Budget", "Gap to Oracle")
