#!/usr/bin/env python3
"""Run BEST-Route adjacent import integration and emit reproducible artifacts."""
...
from verify_best_route_import import _load_config, verify_best_route_import

REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_IMPORT_CONFIG = REPO_ROOT / "configs" / "best_route_official_import_v1.json"
DEFAULT_CONTRACT_CONFIG = REPO_ROOT / "configs" / "best_route_adjacent_comparison_contract_v1.json"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "outputs" / "best_route_adjacent_integration"


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run BEST-Route adjacent integration contract")
...
def main() -> None:
    ...
    for canonical_dataset, spec_any in datasets.items():
        ...
        expected_budgets_raw = spec.get("expected_budgets", [])
        expected_budgets = {int(x) for x in expected_budgets_raw} if isinstance(expected_budgets_raw, list) else set()
                ...
                comparison_rows.append(
                    {
                        ...
                        "source": "scripts/verify_best_route_import.py",
                    }
                )
    ...
    status_payload = {
        ...
        "safe_claims_now": [
            "BEST-Route adjacent import comparison rows are reproducibly exportable for validated datasets.",
            "Control-space equivalence is not claimed; rows remain adjacent_only.",
        ],
        "unsafe_claims_now": [
            "Full upstream BEST-Route reproduction in this repository.",
    ...
    manifest = {
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "run_id": run_id,
        "script": "scripts/run_best_route_adjacent_integration.py",
        ...
        "commands": [
            "python scripts/run_best_route_adjacent_integration.py --import-config configs/best_route_official_import_v1.json --contract-config configs/best_route_adjacent_comparison_contract_v1.json"
        ],
        "artifacts": [
            "manifest.json",
            "status.json",
