#!/usr/bin/env python3
"""Resumable heavy-path generator for allocation-native targets.

Outputs two allocation-native artifacts:
- outside-option labels (branch-vs-outside per state_id),
- pairwise branch preferences (anchor-branch vs peer branches per state_id).

Operational behavior mirrors the oracle stop-vs-act heavy path:
- deterministic replay from state manifest provenance,
- resumable append mode with duplicate protection,
- periodic progress checkpoints,
- per-state error JSONL,
- shard split-manifest compatibility checks.
"""
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Heavy-path allocation target generator (outside + pairwise)")
    ...
    p.add_argument("--outside-labels-out", default="")
    p.add_argument("--pairwise-labels-out", default="")
...
def main() -> None:
    ...
    outside_path = Path(args.outside_labels_out) if args.outside_labels_out else out_dir / "outside_allocation_labels.jsonl"
    pairwise_path = Path(args.pairwise_labels_out) if args.pairwise_labels_out else out_dir / "pairwise_allocation_labels.jsonl"
    manifest_path = Path(args.manifest_out) if args.manifest_out else out_dir / "allocation_label_manifest.json"
    progress_path = Path(args.progress_out) if args.progress_out else out_dir / "allocation_label_progress.json"
    state_errors_path = Path(args.state_errors_out) if args.state_errors_out else out_dir / "allocation_label_state_errors.jsonl"
    ...
    if args.expected_state_count > 0 and len(manifest_rows) != int(args.expected_state_count):
        raise SystemExit(f"Expected {int(args.expected_state_count)} rows in state manifest, got {len(manifest_rows)}")
    ...
    if args.split_manifest:
        ...
        shard_provenance = _validate_shard_spec(split_manifest_path=Path(args.split_manifest), shard_name=str(args.shard_name), rows=manifest_rows)
    ...
    pair_done_state_ids: set[str] = set()
    if args.resume and outside_path.exists():
        for row in _read_jsonl(outside_path):
            state_id = str(row.get("state_id", ""))
            if state_id:
                completed_state_ids.add(state_id)
    if args.resume and pairwise_path.exists():
        for row in _read_jsonl(pairwise_path):
            state_id = str(row.get("anchor_state_id", ""))
            if state_id:
                pair_done_state_ids.add(state_id)
    ...
    def write_progress(last_state_id: str) -> None:
        payload = {
            "generator_impl": "allocation_target_generator_heavy_v1",
            ...
            "pairwise_anchor_states_written": len(pair_done_state_ids),
            "pairwise_rows_written_this_run": pair_rows_written,
    ...
    for idx, row in enumerate(manifest_rows):
        ...
        if state_id in completed_state_ids and state_id in pair_done_state_ids:
            resume_skips += 1
        ...
        try:
            ...
            if state_id not in completed_state_ids:
                act_vals: list[float] = []
                ...
                for rollout_idx in range(paired_rollouts):
                    ...
                    out_rng = random.Random(paired_seed)
                    v_here = _local_rollout_value(
                    ...
                    act_vals.append(float(v_here))
                    outside_vals.append(float(v_out))
                    gap_vals.append(float(v_here - v_out))

                q_here = _mean(act_vals)
                q_out = _mean(outside_vals)
                gap = q_here - q_out
                outside_row = {
                    "state_id": state_id,
                    ...
                    "q_branch": q_here,
                    "q_outside": q_out,
                    "outside_option_gap": gap,
                    "label_branch_beats_outside": 1 if gap > 0 else 0,
                    "gap_std": _std(gap_vals, _mean(gap_vals)),
                    "agreement_rate": _mean([1.0 if ((g > 0) == (gap > 0)) else 0.0 for g in gap_vals]),
                    "rollout_count": paired_rollouts,
                    "generator_impl": "allocation_target_generator_heavy_v1",
                    "shard_name": str(args.shard_name),
                    "shard_id": int(args.shard_id),
                }
                _append_jsonl(outside_path, outside_row)
                completed_state_ids.add(state_id)

            if state_id not in pair_done_state_ids:
                peers = [b.branch_id for b in active_snapshot if b.branch_id != branch_id]
                for peer_id in peers:
                    pref_vals: list[float] = []
                    for rollout_idx in range(paired_rollouts):
                        seed_a = _stable_int_seed(args.seed, state_id, peer_id, rollout_idx, "a")
                        seed_b = _stable_int_seed(args.seed, state_id, peer_id, rollout_idx, "b")
                        v_a = _local_rollout_value(
                            active_snapshot=active_snapshot,
                            forced_first_branch_id=branch_id,
                            skip_first_branch_id=None,
                            horizon_steps=horizon,
                        ...
                        v_b = _local_rollout_value(
                            active_snapshot=active_snapshot,
                            forced_first_branch_id=peer_id,
                            skip_first_branch_id=None,
                            horizon_steps=horizon,
                        ...
                        pref_vals.append(float(v_a - v_b))

                    mean_pref = _mean(pref_vals)
                    pair_row = {
                        ...
                        "branch_b_id": peer_id,
                        "q_a_minus_b": mean_pref,
                        "pairwise_preference": 1 if mean_pref > 0 else (-1 if mean_pref < 0 else 0),
                        "margin_std": _std(pref_vals, mean_pref),
                        "rollout_count": paired_rollouts,
                        "generator_impl": "allocation_target_generator_heavy_v1",
                        "shard_name": str(args.shard_name),
                        "shard_id": int(args.shard_id),
                    }
                    _append_jsonl(pairwise_path, pair_row)
                    pair_rows_written += 1
                pair_done_state_ids.add(state_id)
            ...
            if max_state_errors and failures > max_state_errors:
                write_progress(state_id)
                raise SystemExit(f"State errors exceeded --max-state-errors ({max_state_errors}); latest state_id={state_id}")
    ...
    outside_rows_written = len(_read_jsonl(outside_path))
    pairwise_rows_written_total = len(_read_jsonl(pairwise_path))
    expected_rows = len(manifest_rows)
    complete = outside_rows_written == expected_rows and len(pair_done_state_ids) == expected_rows

    run_manifest = {
        "generator_contract": "allocation_target_generator_interface_v1",
        "generator_impl": "allocation_target_generator_heavy_v1",
        "heavy_path": True,
        "inputs": {
        ...
        "targets": {
            "outside_option": {
                ...
                "rows_written": outside_rows_written,
                "rows_expected_from_manifest": expected_rows,
                "state_key": "state_id",
            },
            "pairwise": {
                "path": str(pairwise_path),
                "rows_written": pairwise_rows_written_total,
                "anchor_states_completed": len(pair_done_state_ids),
                "anchor_state_key": "anchor_state_id",
            },
        },
        "shard": {
            "shard_name": str(args.shard_name),
        ...
        "outputs": {
            "outside_labels_jsonl": str(outside_path),
            "pairwise_labels_jsonl": str(pairwise_path),
    ...
    print(json.dumps({
        "outside_rows_written": outside_rows_written,
        "outside_rows_expected": expected_rows,
        "pairwise_rows_written": pairwise_rows_written_total,
        "complete": complete,
        "state_errors": failures,
        "outside_labels": str(outside_path),
        "pairwise_labels": str(pairwise_path),
