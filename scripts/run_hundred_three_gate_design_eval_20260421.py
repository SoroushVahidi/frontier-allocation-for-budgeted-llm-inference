#!/usr/bin/env python3
"""Six-way eval on frozen 100-case current-full-vs-reasoning_beam2 failures.

Compares:
A) baseline current canonical full
B) hard forced depth-2
C) hard forced depth-3
D) depth2 then gate v1 optimistic collapse-first
E) depth2 then gate v2 budget-aware rescue
F) depth2 then gate v3 ambiguity-after-depth2
"""
...
def main() -> None:
    ...
    out_dir = REPO_ROOT / f"outputs/hundred_three_gate_design_eval_{ts}"
    out_dir.mkdir(parents=True, exist_ok=True)

    baseline_method = TW._resolve_current_full_method()
    methods = {
        "baseline": baseline_method,
        "depth2": DEPTH2_METHOD,
        "depth3": DEPTH3_METHOD,
        "gate1": GATE1_METHOD,
        "gate2": GATE2_METHOD,
        "gate3": GATE3_METHOD,
    }
    ...
    for rec in per_case_in:
        ...
        if not gate_thresholds_snapshot:
            gate_thresholds_snapshot = {
                "gate1": (raw_runs["gate1"].get("metadata") or {}).get("conditional_depth3_gate_thresholds"),
                "gate2": (raw_runs["gate2"].get("metadata") or {}).get("conditional_depth3_gate_thresholds"),
                "gate3": (raw_runs["gate3"].get("metadata") or {}).get("conditional_depth3_gate_thresholds"),
            }
        ...
        row = {
            ...
            **{f"{k}_verifications": int(raw_runs[k]["verifications"]) for k in methods},
            "depth2_release_impossible_under_budget": bool(
                (raw_runs["depth2"].get("metadata") or {}).get("hard_early_coverage_budget_released_impossible")
            ),
            "depth3_release_impossible_under_budget": bool(
                (raw_runs["depth3"].get("metadata") or {}).get("hard_early_coverage_budget_released_impossible")
            ),
            "outcome_depth2_vs_baseline": _delta(cls["baseline"]["correct"], cls["depth2"]["correct"]),
            "outcome_depth3_vs_baseline": _delta(cls["baseline"]["correct"], cls["depth3"]["correct"]),
            "outcome_gate1_vs_baseline": _delta(cls["baseline"]["correct"], cls["gate1"]["correct"]),
            "outcome_gate2_vs_baseline": _delta(cls["baseline"]["correct"], cls["gate2"]["correct"]),
            "outcome_gate3_vs_baseline": _delta(cls["baseline"]["correct"], cls["gate3"]["correct"]),
            "outcome_gate1_vs_depth2": _delta(cls["depth2"]["correct"], cls["gate1"]["correct"]),
            "outcome_gate1_vs_depth3": _delta(cls["depth3"]["correct"], cls["gate1"]["correct"]),
            "outcome_gate2_vs_depth2": _delta(cls["depth2"]["correct"], cls["gate2"]["correct"]),
            "outcome_gate2_vs_depth3": _delta(cls["depth3"]["correct"], cls["gate2"]["correct"]),
            "outcome_gate3_vs_depth2": _delta(cls["depth2"]["correct"], cls["gate3"]["correct"]),
            "outcome_gate3_vs_depth3": _delta(cls["depth3"]["correct"], cls["gate3"]["correct"]),
            "outcome_gate1_vs_gate2": _delta(cls["gate2"]["correct"], cls["gate1"]["correct"]),
            "outcome_gate1_vs_gate3": _delta(cls["gate3"]["correct"], cls["gate1"]["correct"]),
            "outcome_gate2_vs_gate3": _delta(cls["gate3"]["correct"], cls["gate2"]["correct"]),
        }
        row.update(_gate_diag(raw_runs["gate1"], "gate1"))
        row.update(_gate_diag(raw_runs["gate2"], "gate2"))
        row.update(_gate_diag(raw_runs["gate3"], "gate3"))
        rows.append(row)
    ...
    for r in rows:
        d = str(r["dataset"])
        by_dataset[d]["n"] += 1
        for m in ("baseline", "depth2", "depth3", "gate1", "gate2", "gate3"):
    ...
    aggregate = {
        ...
        "release_impossible_under_budget": {
            ...
            "depth2": cnt(lambda r: r["depth2_release_impossible_under_budget"]),
            "depth3": cnt(lambda r: r["depth3_release_impossible_under_budget"]),
            "gate1": cnt(lambda r: r["gate1_release_impossible_under_budget"]),
            "gate2": cnt(lambda r: r["gate2_release_impossible_under_budget"]),
            "gate3": cnt(lambda r: r["gate3_release_impossible_under_budget"]),
        },
        "outcomes_vs_baseline": {m: ocount(f"outcome_{m}_vs_baseline") for m in ("depth2", "depth3", "gate1", "gate2", "gate3")},
        "head_to_head": {
            "gate1_vs_depth2": ocount("outcome_gate1_vs_depth2"),
            "gate1_vs_depth3": ocount("outcome_gate1_vs_depth3"),
            "gate2_vs_depth2": ocount("outcome_gate2_vs_depth2"),
            "gate2_vs_depth3": ocount("outcome_gate2_vs_depth3"),
            "gate3_vs_depth2": ocount("outcome_gate3_vs_depth2"),
            "gate3_vs_depth3": ocount("outcome_gate3_vs_depth3"),
            "gate1_vs_gate2": ocount("outcome_gate1_vs_gate2"),
            "gate1_vs_gate3": ocount("outcome_gate1_vs_gate3"),
            "gate2_vs_gate3": ocount("outcome_gate2_vs_gate3"),
        },
        "gate_decisions": {
            "gate1": ocount("gate1_gate_decision"),
            "gate2": ocount("gate2_gate_decision"),
            "gate3": ocount("gate3_gate_decision"),
        },
        "gate_forces_depth3": {
            "gate1": cnt(lambda r: r["gate1_depth3_forcing_happened"]),
            "gate2": cnt(lambda r: r["gate2_depth3_forcing_happened"]),
            "gate3": cnt(lambda r: r["gate3_depth3_forcing_happened"]),
        },
        "gate_stops_after_depth2": {
            "gate1": cnt(lambda r: (not r["gate1_depth3_forcing_happened"])),
            "gate2": cnt(lambda r: (not r["gate2_depth3_forcing_happened"])),
            "gate3": cnt(lambda r: (not r["gate3_depth3_forcing_happened"])),
        },
        "gate_criteria_fired_counts": {
            "gate1": _criteria_counts(rows, "gate1_gate_triggered_criteria"),
            "gate2": _criteria_counts(rows, "gate2_gate_triggered_criteria"),
            "gate3": _criteria_counts(rows, "gate3_gate_triggered_criteria"),
        },
        ...
        "cases_gate_retains_depth3_gains_with_lower_budget_stress": [
            ...
            for r in rows
            if r["outcome_depth3_vs_baseline"] == "improved"
            and any(r[f"outcome_{g}_vs_baseline"] == "improved" and r[f"{g}_actions"] <= r["depth3_actions"] for g in ("gate1", "gate2", "gate3"))
        ],
        "cases_clearly_wrong_gate_decisions": [
            {
                "dataset": r["dataset"],
                "example_id": r["example_id"],
                "gate1": r["outcome_gate1_vs_depth3"],
                "gate2": r["outcome_gate2_vs_depth3"],
                "gate3": r["outcome_gate3_vs_depth3"],
            }
            for r in rows
            if any(r[f"outcome_{g}_vs_depth3"] == "worsened" for g in ("gate1", "gate2", "gate3"))
        ],
    }

    manifest = {
        "artifact_family": "hundred_three_gate_design_eval",
    ...
    report = REPO_ROOT / f"docs/HUNDRED_CASE_THREE_GATE_DESIGN_COMPARISON_{ts}.md"
    table_lines = [
    ...
    for metric in ("absent_from_tree", "present_not_selected", "repeated_same_family_present", "gold_in_tree"):
        m = aggregate[metric]
        table_lines.append(
            f"| {metric} | {m['baseline']} | {m['depth2']} | {m['depth3']} | {m['gate1']} | {m['gate2']} | {m['gate3']} |"
        )
    md = [
        f"# Hundred-case three-gate design comparison ({ts})",
        "",
        ...
        *table_lines,
        ...
        f"- gate1 vs depth2: {aggregate['head_to_head']['gate1_vs_depth2']}",
        f"- gate1 vs depth3: {aggregate['head_to_head']['gate1_vs_depth3']}",
        f"- gate2 vs depth2: {aggregate['head_to_head']['gate2_vs_depth2']}",
        f"- gate2 vs depth3: {aggregate['head_to_head']['gate2_vs_depth3']}",
        f"- gate3 vs depth2: {aggregate['head_to_head']['gate3_vs_depth2']}",
        f"- gate3 vs depth3: {aggregate['head_to_head']['gate3_vs_depth3']}",
        f"- gate1 vs gate2: {aggregate['head_to_head']['gate1_vs_gate2']}",
        f"- gate1 vs gate3: {aggregate['head_to_head']['gate1_vs_gate3']}",
        f"- gate2 vs gate3: {aggregate['head_to_head']['gate2_vs_gate3']}",
        "",
