#!/usr/bin/env python3
"""Build a reviewer-facing comparison bundle from one or more L1 baseline runs."""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Create L1 baseline comparison bundle")
    p.add_argument("--run-dirs", required=True, help="Comma-separated L1 run directories")
    p.add_argument("--output-dir", default="outputs/l1_baseline/comparison_bundle")
    return p.parse_args()
...
def main() -> None:
    ...
    note_lines = [
        "# L1 baseline comparison bundle",
        "",
        f"- generated_at_utc: `{bundle_manifest['created_utc']}`",
        f"- num_input_runs: `{bundle_manifest['num_runs']}`",
        "",
        "## Scope",
        "- This bundle merges L1 baseline runs for manuscript table assembly.",
        "- Primary fair comparisons are MODE A (`external_l1_exact` / `external_l1_max`) vs our anchor under matched-budget rows.",
        "- MODE B rows remain secondary and separately labeled due potential RL/post-training differences.",
    ]
