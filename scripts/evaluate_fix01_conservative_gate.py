#!/usr/bin/env python3
"""
Evaluate FIX-01 conservative two-stage gate.
...
OUT_DIR = REPO_ROOT / "outputs" / "fix01_conservative_gate_20260524"
...
def estimate_hierarchical_priors(train_df: pd.DataFrame) -> Dict[str, Tuple[float, float]]:
    """
    Empirical-Bayes prior pooling across available scenario cells in train_df.
    Produces per-source (alpha0, beta0) with conservative shrinkage.
    """
    priors: Dict[str, Tuple[float, float]] = {}
    if "scenario_id" not in train_df.columns:
        return {src: (1.0, 1.0) for src in SOURCES}
    ...
    for src in SOURCES:
        ...
        by_cell = train_df.groupby("scenario_id")[col].mean().dropna()
        if len(by_cell) < 2:
            priors[src] = (1.0, 1.0)
            continue

        m = float(by_cell.mean())
        v = float(by_cell.var(ddof=1))
        # Method of moments for Beta prior; clamp to keep conservative.
        if v <= 1e-6 or m <= 1e-6 or m >= 1.0 - 1e-6:
            priors[src] = (1.0, 1.0)
            continue
        ...
        # Conservative cap: avoid overly confident priors on small cell counts.
        k = float(min(k, 20.0))
        a0 = max(1.0, m * k)
        b0 = max(1.0, (1.0 - m) * k)
        priors[src] = (a0, b0)
...
def build_calibration(train_df: pd.DataFrame) -> Calibration:
    n = int(len(train_df))
    priors = estimate_hierarchical_priors(train_df)
    posteriors: Dict[str, SourcePosterior] = {}
    for src in SOURCES:
        n_ok = int(train_df[f"{src}_ok"].sum())
        a0, b0 = priors.get(src, (1.0, 1.0))
        p = beta_posterior_bounds(n_ok, n, alpha0=a0, beta0=b0)
        p.source = src
        posteriors[src] = p
...
def coalition_features(row: pd.Series, majority_answer: str) -> Dict[str, int]:
    members = []
    for src in SOURCES:
        if _safe_str(row.get(f"{src}_ans")) == majority_answer:
            members.append(src)
    size = len(members)
    includes_frontier = int("frontier" in members)
    external_only = int(size > 0 and includes_frontier == 0)
    # With current pool, diversity means coalition spans frontier + external family.
    diversity = int(size > 0 and includes_frontier == 1 and size >= 2)
    return {
        "coalition_size": size,
        "includes_frontier": includes_frontier,
        "external_only": external_only,
        "diversity": diversity,
    }


def dominant_by_probability_with_gap(calib: Calibration, tau: float, delta: float, seed: int) -> Tuple[Optional[str], Dict[str, float]]:
    probs = {}
    for src in SOURCES:
        probs[src] = probability_of_dominance(calib, src, delta=delta, n_samples=2000, seed=seed + _seed_from_key("domprob", src))
    best = max(SOURCES, key=lambda s: probs[s])
    mean_gap = calib.posteriors[best].mean - calib.posteriors[calib.ranked[1]].mean
    if probs[best] >= tau and mean_gap >= delta:
        return best, probs
    return None, probs


def conservative_override_condition(
    row: pd.Series,
    calib: Calibration,
    dominant_src: str,
    d_prob: float,
    ...
    majority_size = int(row.get("majority_size", 0))
    feats = coalition_features(row, majority_answer)
    dom_ans = _safe_str(row.get(f"{dominant_src}_ans"))
    if not dom_ans:
        return False

    # Borderline zone: majority can override only if dominance isn't clearly strong.
    borderline = d_prob < min(0.99, tau + 0.10)
    if not borderline:
        return False

    margin = _weighted_vote_margin(row, calib, majority_answer, dom_ans)

    if majority_size >= 3:
        if feats["external_only"] and d_prob >= max(tau, 0.85):
            return False
        if feats["external_only"]:
            return margin >= (override_margin + external_corr_penalty)
        return feats["diversity"] == 1 and margin >= override_margin

    if majority_size == 2:
        # Never allow external-only pair to override strong dominant signal.
        if feats["external_only"]:
            return False
        second_best = calib.ranked[1]
        second_ans = _safe_str(row.get(f"{second_best}_ans"))
        includes_second = second_ans == majority_answer
        return bool(includes_second and feats["includes_frontier"] and margin >= override_margin)
...
def infer_strengthened_c1d(
    ...
    if variant == "fix01a":
        ...
        dominant_src = params.get("_dominant_src", None)
        if "_dominant_src" not in params:
            dom, probs = dominant_by_probability_with_gap(
                calib,
                tau=params.get("tau", 0.90),
                delta=params.get("delta", 0.02),
                seed=rng_seed,
            )
            dominant_src = dom
            params["_dom_probs"] = probs
        d_probs = params.get("_dom_probs", {})
    else:
    ...
    if variant == "fix01f":
        d_prob = float(params.get("_dom_probs", {}).get(dominant_src, 0.0))
        override_passed = conservative_override_condition(
            row=row,
            calib=calib,
            ...
            tau=params.get("tau", 0.90),
            delta=params.get("delta", 0.02),
            override_margin=params.get("override_margin", 0.05),
            external_corr_penalty=params.get("external_corr_penalty", 0.03),
        )
    else:
        ...
        for delta in [0.02, 0.03]:
            for ov in [0.05, 0.08]:
                variants.append(
                    {
                        "variant_name": (
                            f"fix01f_conservative_tau{int(tau*100):02d}"
                            f"_d{int(delta*100):02d}_ov{int(ov*100):02d}"
                        ),
                        "family": "fix01f",
                        "params": {
                            "tau": tau,
...
def evaluate_fold(
    ...
    for v in variant_grid:
        ...
        if family == "fix01a":
            ...
            gate = near_peer_safety_gate(
                calib,
                params.get("spread_min", 0.05),
                entropy_max=params.get("entropy_max"),
            )
            params["_near_peer_blocked"] = not gate
            if gate:
                seed = _seed_from_key(protocol, fold_id, v["variant_name"], "fold_dominance")
                ...
                params["_dominant_src"] = dom
                params["_dom_probs"] = probs
            else:
                params["_dominant_src"] = None
                params["_dom_probs"] = {}
...
def build_report(
    ...
    doc = REPO_ROOT / "docs" / "FIX01_CONSERVATIVE_GATE_20260524.md"
    ...
    with open(doc, "w", encoding="utf-8") as f:
        f.write("# FIX01_CONSERVATIVE_GATE_20260524\n\n")
        ...
        f.write(
            "Implemented families: FIX01a (LCB-gap), FIX01b (prob-of-dominance), "
            "FIX01c (near-peer safety), FIX01d (pattern-aware), FIX01e (minimal safe), "
            "FIX01f (two-stage conservative gate).\n\n"
        )
        ...
        f.write("See `outputs/fix01_conservative_gate_20260524/fix01_best_variant_casebook.md` and companion CSVs.\n\n")
...
def main():
    ...
    write_manifest(
        ...
        scripts_created=["scripts/evaluate_fix01_conservative_gate.py"],
        output_files=output_files,
    )
