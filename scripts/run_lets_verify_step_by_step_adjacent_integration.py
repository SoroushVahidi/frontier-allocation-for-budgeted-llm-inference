#!/usr/bin/env python3
"""Run stable Let's Verify Step by Step adjacent integration lane."""
...
from verify_lets_verify_step_by_step_import import verify_lets_verify_step_by_step_import

REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_CONTRACT = REPO_ROOT / "configs" / "lets_verify_step_by_step_adjacent_comparison_contract_v1.json"
DEFAULT_OUTPUT_ROOT = REPO_ROOT / "outputs" / "lets_verify_step_by_step_adjacent_integration"
DEFAULT_OFFICIAL_REPO_PATH = Path("/tmp/lets_verify_upstream/prm800k")
...
def _try_clone_if_missing(path: Path) -> dict[str, Any]:
    ...
    cmd = ["git", "clone", "--depth", "1", "https://github.com/openai/prm800k.git", str(path)]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run stable Let's Verify Step by Step adjacent integration lane")
...
def main() -> None:
    ...
    commands = [
        f"python scripts/run_lets_verify_step_by_step_adjacent_integration.py --contract-config {contract_path.relative_to(REPO_ROOT)}"
    ]
    ...
    for subset in stable_subsets:
        ...
        report = verify_lets_verify_step_by_step_import(
                ...
                comparison_rows.append(
                    {
                        ...
                        "verifier_score_threshold": row.get("verifier_score_threshold", ""),
    ...
    status_payload = {
        ...
        "canonical_title": contract.get("canonical_title", "Let's Verify Step by Step"),
        "classification": classification,
    ...
    summary_md = "\n".join(
        [
            "# Let's Verify Step by Step adjacent integration summary",
            "",
            f"- Run ID: `{run_id}`",
            ...
            "## Scope guardrail",
            "",
            "- This run is **adjacent-only** and does not claim full faithful in-repo reproduction.",
            "- Claims remain limited to validated import contract + official-source layout checks.",
        ]
    ) + "\n"
    ...
    manifest = {
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "run_id": run_id,
        "script": "scripts/run_lets_verify_step_by_step_adjacent_integration.py",
        "output_dir": str(run_dir.relative_to(REPO_ROOT)),
        "commands": commands,
