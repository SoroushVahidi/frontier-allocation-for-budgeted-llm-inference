#!/usr/bin/env python3
"""Canonical hundred-case failure statistics for strict_gate1_cap_k6 vs best exact-loss comparator.
...
Finalized default method is fixed to strict_gate1_cap_k6 and comparator is resolved on the
same exact-loss surface by direct adversary strength.
"""
...
STRICT_PHASED_STATUS_DOC = REPO_ROOT / "docs/CURRENT_DEFAULT_MODEL_AND_STRICT_PHASED_STATUS_2026_04_21.md"
EXACT_LOSS_COMPARATOR_DOC = REPO_ROOT / "docs/CURRENT_RANKING_AND_COMPETITIVE_STATUS_2026_04_20.md"
TARGET_METHOD_ALIAS = "broad_diversity_aggregation_strong_v1_anti_collapse_answer_group_refinement_repeat_expansion_fine_incumbent_guard_tuned_v1_hard_early_root_depth2_then_gate_v1_optimistic_collapse_first_hard_max_family_expansions_cap_k6_v1__deterministic_output_layer_repair_v1"


def _resolve_target_method() -> tuple[str, dict[str, Any]]:
    return (
        TARGET_METHOD_ALIAS,
        {
            "selection_rule": "Use finalized broad default method from strict-phased default decision/status docs",
            "strict_phased_default_doc": str(STRICT_PHASED_DEFAULT_DOC.relative_to(REPO_ROOT)),
            "strict_phased_status_doc": str(STRICT_PHASED_STATUS_DOC.relative_to(REPO_ROOT)),
            "resolved_method": TARGET_METHOD_ALIAS,
            "target_short_name": "strict_gate1_cap_k6",
        },
    )


def _resolve_best_comparator_for_target(current_method: str) -> tuple[str, dict[str, Any]]:
    hinted = "reasoning_beam2"
    if EXACT_LOSS_COMPARATOR_DOC.exists():
        txt = EXACT_LOSS_COMPARATOR_DOC.read_text(encoding="utf-8")
        m = re.search(
            r"direct adversary on that fresh loss-set surface is `([^`]+)`",
            txt,
            flags=re.IGNORECASE,
        )
        if m:
    ...
    best, base_evidence = TW._resolve_best_method(current_method)
    scored = base_evidence.get("candidate_correct_counts", {})
    return (
        best,
        {
            "selection_rule": "Choose strongest direct adversary from canonical exact-loss comparator sweep (`build_twenty_exact_current_full_vs_best_fresh.py`) and validate against canonical direct-adversary doc hint.",
            "exact_loss_evidence_doc": str(EXACT_LOSS_COMPARATOR_DOC.relative_to(REPO_ROOT)),
            "hinted_direct_adversary_from_doc": hinted,
            "candidate_correct_counts": scored,
            "base_resolution_evidence": base_evidence,
            "resolved_best_comparator": best,
        },
    )
...
def _collect_excluded_case_ids_extended() -> set[str]:
    ...
    for p in REPO_ROOT.glob(
        "outputs/hundred_current_full_vs_best_failure_statistics_*/selection_manifest.json"
    ):
        data = json.loads(p.read_text(encoding="utf-8"))
        for c in data.get("cases", []):
            if "case_id" in c:
...
def _run_one_case(
    ...
    best_raw = _run_observed_with_events(best_method, row, "fresh_best")
    ...
    compact_row = {
        ...
        "seed": int(row["seed"]),
        "budget": int(row["budget"]),
        "selected_support_count": int(pick["loss_support_count"]),
...
def _build_top10_feature_analysis(
    per_case: list[dict[str, Any]], flat_rows: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    n = len(per_case)
    by_ft = Counter(str(p["failure_type"]) for p in per_case)
    repeated_n = sum(
        1 for p in per_case if p["same_family_expansion_severity"]["repeated_same_family_present"]
    )
    absent_n = by_ft.get("absent_from_tree", 0)
    present_n = by_ft.get("present_not_selected", 0)
    output_n = by_ft.get("output_or_extraction_mismatch", 0)
    dominant_share = [
        float(p["same_family_expansion_severity"].get("max_family_share_of_expansions") or 0.0)
        for p in per_case
    ]
    long_runs = [
        int(p["same_family_expansion_severity"].get("longest_consecutive_same_family_run") or 0)
        for p in per_case
    ]
    ags = [int(r.get("num_answer_groups", 0) or 0) for r in flat_rows]
    monopoly_n = sum(
        1
        for p in per_case
        if p["alternative_answer_group_count_and_maturity"].get("one_answer_group_monopolized_tree")
    )
    better_sel = sum(
        1 for p in per_case if "better_selection" in list(p.get("best_method_advantage_type", []))
    )
    earlier = sum(
        1 for p in per_case if "earlier_correct_entry" in list(p.get("best_method_advantage_type", []))
    )
    less_collapse = sum(
        1 for p in per_case if "less_collapse" in list(p.get("best_method_advantage_type", []))
    )
    budget_eff = sum(
        1 for p in per_case if "better_budget_efficiency" in list(p.get("best_method_advantage_type", []))
    )
    features = [
        {
            "rank": 1,
            "feature": "failure_type_absent_from_tree",
            "definition": "Gold answer absent from strict_gate1_cap_k6 final tree under deterministic repair replay.",
            "prevalence_n": absent_n,
            "prevalence_pct": 100.0 * absent_n / max(1, n),
            "why_important": "Coverage remains the dominant residual failure mode.",
            "improvement_direction": "Increase early family diversity and targeted entry to gold-bearing branches.",
        },
        {
            "rank": 2,
            "feature": "failure_type_present_not_selected",
            "definition": "Gold exists in our tree but final selected answer is wrong.",
            "prevalence_n": present_n,
            "prevalence_pct": 100.0 * present_n / max(1, n),
            "why_important": "Large secondary mode indicates selection/scoring opportunity.",
            "improvement_direction": "Tune answer-group ranking and tie-break logic when gold is present.",
        },
        {
            "rank": 3,
            "feature": "repeated_same_family_expansion_present",
            "definition": "Observed repeated expansion in the same root family via event-level replay.",
            "prevalence_n": repeated_n,
            "prevalence_pct": 100.0 * repeated_n / max(1, n),
            "why_important": "Family concentration often co-occurs with absent-from-tree failures.",
            "improvement_direction": "Strengthen anti-collapse gating under cap pressure.",
        },
        {
            "rank": 4,
            "feature": "dominant_family_share_high",
            "definition": "Max family share of expansions >=0.60 in a case.",
            "prevalence_n": sum(1 for x in dominant_share if x >= 0.60),
            "prevalence_pct": 100.0 * sum(1 for x in dominant_share if x >= 0.60) / max(1, n),
            "summary_stat": {"mean": round(statistics.mean(dominant_share), 4)},
            "why_important": "Expansion mass concentration reduces breadth and gold-entry probability.",
            "improvement_direction": "Raise penalties for dominant-family reuse before F3-like depth.",
        },
        {
            "rank": 5,
            "feature": "longest_same_family_run_ge3",
            "definition": "Longest consecutive same-family expansion run is >=3.",
            "prevalence_n": sum(1 for x in long_runs if x >= 3),
            "prevalence_pct": 100.0 * sum(1 for x in long_runs if x >= 3) / max(1, n),
            "summary_stat": {"p50": statistics.median(long_runs)},
            "why_important": "Long runs indicate local myopia episodes.",
            "improvement_direction": "Add run-length-aware rescue branching.",
        },
        {
            "rank": 6,
            "feature": "answer_group_monopoly",
            "definition": "One answer group monopolizes support while alternatives exist.",
            "prevalence_n": monopoly_n,
            "prevalence_pct": 100.0 * monopoly_n / max(1, n),
            "summary_stat": {"mean_num_answer_groups": round(statistics.mean(ags), 4)},
            "why_important": "Group monopoly can suppress late-correct minority branches.",
            "improvement_direction": "Require challenger-group verification when monopoly signal fires.",
        },
        {
            "rank": 7,
            "feature": "comparator_earlier_correct_entry",
            "definition": "Comparator shows earlier correct entry advantage tag.",
            "prevalence_n": earlier,
            "prevalence_pct": 100.0 * earlier / max(1, n),
            "why_important": "Comparator wins by entering correct basin sooner.",
            "improvement_direction": "Bias first successful deepening toward under-explored families.",
        },
        {
            "rank": 8,
            "feature": "comparator_better_budget_efficiency",
            "definition": "Comparator reaches correctness with materially fewer actions.",
            "prevalence_n": budget_eff,
            "prevalence_pct": 100.0 * budget_eff / max(1, n),
            "why_important": "Budget-use inefficiency leaves less room for corrective exploration.",
            "improvement_direction": "Reallocate verification vs expansion under cap_k6.",
        },
        {
            "rank": 9,
            "feature": "comparator_less_collapse",
            "definition": "Comparator has lower family-collapse concentration signal.",
            "prevalence_n": less_collapse,
            "prevalence_pct": 100.0 * less_collapse / max(1, n),
            "why_important": "Confirms collapse as practical failure accelerator.",
            "improvement_direction": "Increase cross-family compulsory hops after repeated concentration.",
        },
        {
            "rank": 10,
            "feature": "selection_advantage_signal",
            "definition": "Comparator tagged with better_selection.",
            "prevalence_n": better_sel,
            "prevalence_pct": 100.0 * better_sel / max(1, n),
            "why_important": "Selection layer still explains a substantial subset.",
            "improvement_direction": "Improve chosen-node confidence calibration and extraction consistency.",
        },
    ]
    # annotate simple cross-tab signal
    for f in features:
        f["cross_tab_failure_type"] = {"absent_from_tree": absent_n, "present_not_selected": present_n, "output_or_extraction_mismatch": output_n}
    return features
...
def main() -> None:
    ...
    out_dir = REPO_ROOT / f"outputs/canonical_hundred_strict_gate1_cap_k6_vs_best_failure_statistics_{ts}"
    out_dir.mkdir(parents=True, exist_ok=True)

    current_method, newest_resolution = _resolve_target_method()
    best_method, comparator_resolution = _resolve_best_comparator_for_target(current_method)

    excluded = _collect_excluded_case_ids_extended()
    rows = _build_eval_surface_hundred(current_method, best_method)
    ...
    selection_policy = {
        **selection_policy,
        ...
        "best_method_selected": best_method,
        "newest_method_selected": current_method,
        "newest_method_resolution": newest_resolution,
        "best_comparator_resolution": comparator_resolution,
        "surface_expansion": {
    ...
    for pick in merged:
        ...
        for row in pick.get("group_rows") or [pick["representative"]]:
            built = _run_one_case(pick, row, current_method, best_method)
    ...
    aggregate = {
        ...
        "cross_tabs": {
            ...
            "failure_type_x_error_geometry": _cross_tab(
                [
                    {
                        **r,
                        "error_geometry": (r.get("error_geometry") or "other").split("|")[0],
                    }
                    for r in flat_rows
                ],
                "failure_type",
    ...
    manifest = {
        "artifact_family": "canonical_hundred_strict_gate1_cap_k6_vs_best_failure_statistics",
        "created_at_utc": now.isoformat(),
        "output_dir": str(out_dir.relative_to(REPO_ROOT)),
        "selected_newest_method_name": current_method,
        "selected_best_comparator_name": best_method,
        "newest_method_resolution": newest_resolution,
        "best_comparator_resolution": comparator_resolution,
        "selection_policy": selection_policy,
        "cases": [
    ...
    top10 = _build_top10_feature_analysis(per_case, flat_rows)
    (out_dir / "top10_most_important_losing_case_features.json").write_text(
        json.dumps(top10, indent=2) + "\n", encoding="utf-8"
    )
    with (out_dir / "top10_most_important_losing_case_features.csv").open(
    ...
    if len(rep_examples) < 10:
        for p in per_case:
            if p not in rep_examples:
                rep_examples.append(p)
            if len(rep_examples) >= 12:
                break
    rep_examples = rep_examples[:15]

    doc_path = REPO_ROOT / f"docs/CANONICAL_HUNDRED_STRICT_GATE1_CAP_K6_VS_BEST_FAILURE_STATISTICS_{ts}.md"
    ...
    lines: list[str] = []
    lines.append("# Canonical hundred-case failure statistics: strict_gate1_cap_k6 vs best exact-loss comparator")
    lines.append("")
    ...
    lines.append(f"- **Selected finalized default method**: `{current_method}`")
    lines.append(f"- **Selected best comparator for exact-loss surface**: `{best_method}`")
    lines.append(
    ...
    lines.append("- **Comparator resolution rule**: compute direct-adversary strength on this method's exact-loss surface by maximizing `(ours wrong && comparator correct)`; tie-break by comparator accuracy and canonical doc hint.")
    lines.append("")
    ...
    lines.append(
        f"1. **Eligibility**: On that grid, keep rows where strict_gate1_cap_k6 is incorrect and `{best_method}` is correct."
    )
    lines.append(
    ...
    lines.append(
        ...
        f"still shows **ours wrong / {best_method} correct** (stream tags `fresh_our` / `fresh_best` match the 20-case builder)."
    )
    lines.append("")
    ...
    lines.append(f"- `{out_dir.relative_to(REPO_ROOT)}/top10_most_important_losing_case_features.json`")
    lines.append(f"- `{out_dir.relative_to(REPO_ROOT)}/top10_most_important_losing_case_features.csv`")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Top 10 most important features of the losing cases")
    lines.append(
        "- Machine-readable source: `top10_most_important_losing_case_features.json` (plus CSV export)."
    )
    for row in top10:
        lines.append(
            f"{row['rank']}. **{row['feature']}** — {row['prevalence_n']}/{n} ({row['prevalence_pct']:.1f}%). "
            f"Definition: {row['definition']} Why it matters: {row['why_important']} Next direction: {row['improvement_direction']}"
        )
    lines.append("")
