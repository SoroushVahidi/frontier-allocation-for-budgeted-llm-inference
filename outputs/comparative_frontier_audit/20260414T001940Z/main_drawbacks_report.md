# Main drawbacks report — comparative frontier audit (`20260414T001940Z`)
...
- Head-to-head cells (dataset × budget × baseline): **wins=1**, **losses=0**, **ties=4** (see `comparison_summary.csv`).

| Baseline | Mean Δ acc (ours − baseline) |
|---|---|
| `program_of_thought` | 0.3333 |
| `reasoning_beam2` | 0.0000 |
| `reasoning_greedy` | 0.0000 |
| `self_consistency_3` | 0.0000 |
...
- Mean **gap to oracle** for `adaptive_min_expand_1`: **0.0000** (oracle = best per-example strategy across all eight families).
...
- `adaptive_min_expand_1` mean realized **avg_actions / budget**: 0.722 (see `method_metrics.csv`).
- Mean **budget_exhaustion_rate** for `adaptive_min_expand_1`: **0.6667**.
...
1. **Anti-collapse knob**: Mean gap-to-oracle across budgets/datasets: k=0 → 0.000, k=1 → 0.000, k=2 → 0.000. If k=1 is not uniformly best, **min-expand is regime-dependent** (see ablation in `method_metrics.csv`).
