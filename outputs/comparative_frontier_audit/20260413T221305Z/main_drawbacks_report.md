# Main drawbacks report — comparative frontier audit (`20260413T221305Z`)
...
- Head-to-head cells (dataset × budget × baseline): **wins=1**, **losses=4**, **ties=0** (see `comparison_summary.csv`).

| Baseline | Mean Δ acc (ours − baseline) |
|---|---|
| `program_of_thought` | -0.2500 |
| `reasoning_beam2` | -0.2500 |
| `reasoning_greedy` | -0.2500 |
| `self_consistency_3` | -0.2500 |
| `verifier_guided_search` | 0.2500 |
...
- Mean **gap to oracle** for `adaptive_min_expand_1`: **0.2500** (oracle = best per-example strategy across all eight families).
...
- `adaptive_min_expand_1` mean realized **avg_actions / budget**: 0.969 (see `method_metrics.csv`).
- Mean **budget_exhaustion_rate** for `adaptive_min_expand_1`: **0.5000**.
...
3. **Anti-collapse knob**: Mean gap-to-oracle across budgets/datasets: k=0 → 0.750, k=1 → 0.250, k=2 → 0.250. If k=1 is not uniformly best, **min-expand is regime-dependent** (see ablation in `method_metrics.csv`).
