...
All promoted comparisons are conducted on \textbf{Cohere command-r-plus-08-2024} with a fixed per-method budget cap of $B = 6$ logical API calls per example.  The dataset is \texttt{openai/gsm8k}.  Evaluation conditions are matched across all compared candidate-producing methods: every method operates under the same budget cap, provider, and model, so accuracy differences are not confounded by unequal per-method compute.
...
FIX-2+FIX-4 adds negligible selection overhead once candidate answers and frontier
...
exceed a single method's $B=6$ run.  In the logged Aggregate720 runs, the frontier
