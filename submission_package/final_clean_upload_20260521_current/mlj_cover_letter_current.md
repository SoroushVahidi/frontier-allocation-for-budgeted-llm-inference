...
For transparency, we disclose the related NeurIPS manuscript *Frontier Allocation for Budgeted LLM Inference* and have uploaded it in the portal's Related files section. That manuscript studies frontier allocation during budgeted inference: a controller maintains active partial reasoning branches and decides which branch to expand or when to commit under a fixed budget. It focuses on branch-level compute allocation during reasoning, including branch-local continuation scoring, answer-support aggregation, tree-shape control, repeat-expansion moderation, and bounded repair. The present *Machine Learning* submission is distinct: it studies post-generation selective deferral for final-answer selection among already generated candidate answers, using logged failure-trace metadata and external-answer agreement under a matched per-producer budget.
