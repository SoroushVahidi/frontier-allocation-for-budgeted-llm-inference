...
## True same-model SC and result registry (2026-07-25)

- Read `AGENTS.md` / `CLAUDE.md` safety rules first for paid API / push constraints.
- Do not delete `outputs/`; archive or mark invalid instead (`INVALID_*.md` + `RESULT_STATUS.json`).
- Update `docs/results/result_registry.yaml` when registering new results.
- True same-model SC must use `experiment_type: true_same_model_self_consistency` with
  `controller_enabled: false` and `repair_override_enabled: false` (see
  `docs/experiments/TRUE_SC_N6_PROTOCOL.md`).
- Run `.venv/bin/pytest tests/test_true_same_model_sc.py` before changing SC infrastructure.
