...
The two gates in Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4) target failure modes that are identifiable at runtime
without any access to gold labels.
The Low-Depth Guard (LDG; implementation: FIX-2) addresses a structural weakness: when the frontier search
...
The External-Consensus Fallback (ECO; implementation: FIX-4) targets a different pattern: when the
...
answer, that unanimous disagreement constitutes strong evidence that the frontier is wrong.
...
\paragraph{Why the not retained variants were abandoned.}
...
compared to the $+8.33$~pp advantage of Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4) over TALE on the same split.
...
FIX-6 (extra-action variant) proposed collecting extra frontier or TALE-retry actions for predicted
failure cases.
...
net $-1$ for extra frontier actions and net $-4$ for extra TALE retries relative to Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4),
with meaningful regressions on the corresponding control slices (8.9\% for frontier, 11.1\% for TALE retry).
...
FIX-7 (cluster-level selector) showed only limited positive movement in offline replay,
with a best-rule local gain of $+0.67$~pp on final-300, but it did not provide
validation-grade aggregate evidence or a robust policy improvement and was therefore
not retained.
...
The result is that the point-estimate advantage of Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4) is consistent across
...
records and frontier trace metadata needed to replay Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4) under the current
protocol.
They are therefore unsuitable as a clean second benchmark/provider/budget result
for this manuscript without new matched-budget generation.
