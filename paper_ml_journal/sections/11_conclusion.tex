...
We presented FIX-2+FIX-4, a deterministic gold-free policy gating method for
matched-budget LLM reasoning.
On a disjoint 720-example aggregate evaluation (three independent seeds, zero overlap),
FIX-2+FIX-4 achieves 80.69\% (581/720) versus L1 77.64\%, S1 77.08\%, and TALE 75.14\%.
...
On the Final-300 evaluation (seed 71, budget 6), FIX-2+FIX-4 reaches 86.67\% (260/300),
...
The Low-Depth Guard (FIX-2) detects structurally weak frontier search branches using the
logged \texttt{override\_reason} field and substitutes the external majority vote.
The External-Consensus Fallback (FIX-4) identifies cases where all three external
...
Table~\ref{tab:policy_rules} and Algorithm~\ref{alg:fix24}, and the complete
implementation is available in \texttt{experiments/support\_aware\_selector.py}.

Further accuracy gains in this setting will most likely require advances in candidate
