...
\paragraph{Frontier metadata interface (runtime, gold-free).}
...
\]
Low-Depth Guard (LDG; implementation: FIX-2) and External-Consensus Fallback (ECO; implementation: FIX-4) are empirical, gold-free heuristics intended to identify such subsets without using correctness labels at runtime.
...
Relative to L1, S1, TALE, and the frontier baseline, Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4) plays a different
role in the stack.
L1, S1, TALE, and frontier each provide candidate answers under the same
per-method matched budget; the failure-trace-guided allocator then decides whether to keep the
...
\subsection{Low-Depth Guard (LDG; implementation: FIX-2)}
\label{sec:fix2}
...
\subsection{External-Consensus Fallback (ECO; implementation: FIX-4)}
\label{sec:fix4}
...
\begin{table}[t]
\centering
\caption{Policy summary for Combined Failure-Trace-Guided Allocator (Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4)). All inputs are gold-free runtime fields. Named constants and name mapping are documented in the appendix reproducibility tables.}
\label{alg:fix24}
\scriptsize
...
\midrule
\multicolumn{2}{l}{\textit{Step 1 ---Low-Depth Guard (LDG; implementation: FIX-2)}} \\
...
\midrule
\multicolumn{2}{l}{\textit{Step 2 ---External-Consensus Fallback (ECO; implementation: FIX-4)}} \\
