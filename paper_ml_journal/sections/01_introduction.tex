...
The main contribution of this paper is \textbf{Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4)},
a deterministic two-gate policy that relies on
failure-trace metadata logged during normal frontier execution.
The \textit{Low-Depth Guard} (LDG; implementation: FIX-2) fires when the frontier's
\texttt{override\_reason} field signals a structurally weak single-branch output,
substituting an external majority vote.
The \textit{External-Consensus Fallback} (ECO; implementation: FIX-4) fires when all three external
...
Failure-Trace Allocator (FTA, implementation: FIX-2+FIX-4) achieves 80.69\% accuracy versus 77.64\% (L1), 77.08\% (S1), and
75.14\% (TALE).
...
development from validation evidence.
