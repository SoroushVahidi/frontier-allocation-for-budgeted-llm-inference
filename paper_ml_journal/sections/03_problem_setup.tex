...
We consider method selection under a fixed budget parameter (budget $=6$) on Cohere command-r-plus-08-2024 runs over \texttt{openai/gsm8k} examples. For each example, baseline methods produce candidate final answers. The objective is to maximize exact-match accuracy without using gold labels at runtime.
...
\begin{description}[leftmargin=2.2cm, labelwidth=2cm, labelsep=0.2cm, style=nextline]
  ...
  \item[L1] \texttt{external\_l1\_max}: an external method that selects the answer with the highest support count across sampled model outputs.
  \item[S1] \texttt{external\_s1\_budget\_forcing}: an external method that applies sequential budget-forcing to elicit a final answer within the allocated budget.
  \item[TALE] \texttt{external\_tale\_prompt\_budgeting}: an external method that uses prompt-level budget cues to guide answer selection.
