...
Class~A count and, separately, the higher of FTA vs.\ Pooled-4---descriptive highlighting, not a
corrected leaderboard. Trust tiers and the Fireworks$\times$GPQA blocked outcome are defined in
Section~\ref{sec:setup}.}
\label{tab:main-matrix}
\input{tables/table1_main_4x4_matrix}
\end{table}
...
Figure~\ref{fig:scatter}). Azure$\times$GSM8K has near-ceiling discovery with a large negative FTA
...
IDs, not a joint cost-matched leaderboard.}
\label{tab:compute-matched-phase7}
\resizebox{\textwidth}{!}{\input{tables/table5_compute_matched_phase7}}
\end{table}
