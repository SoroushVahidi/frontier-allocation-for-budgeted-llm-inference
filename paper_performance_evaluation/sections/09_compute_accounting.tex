...
note.}
\label{tab:resource}
\input{tables/table3_resource_accounting}
\end{table}
...
\caption{Accounting status for resource fields used in Table~\ref{tab:resource} and
Figure~\ref{fig:tradeoff}. The status labels distinguish exact, reconstructed, incomplete, and
lower-bound quantities; no missing token, retry, or dollar fields are imputed.}
\label{tab:accounting-status}
\input{tables/table9_accounting_status}
\end{table}

Table~\ref{tab:resource} reports unweighted means of per-cell means, and
Table~\ref{tab:accounting-status} records which resource fields support exact versus lower-bound
interpretations. Class~B end-to-end cost inherits up to four Class~A pipelines.
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.86\linewidth,trim=8 8 8 8,clip]{figures/figure4_accuracy_vs_cost_tradeoff}
\caption{Incomplete-telemetry diagnostic for Class~A accuracy and mean estimated cost per example
(milli-USD). Open Frontier markers with rightward arrows indicate that challenger-phase token
fields were not persisted, so Frontier cost positions are lower bounds. This diagnostic should not
be read as a Pareto frontier: policies are not resource-matched, and paid retries are incomplete.}
\label{fig:tradeoff}
\end{figure}

Figure~\ref{fig:tradeoff} is therefore a diagnostic for incomplete cost telemetry, not a definitive
cost-efficiency frontier. The single-cell Azure$\times$GSM8K SC comparison
