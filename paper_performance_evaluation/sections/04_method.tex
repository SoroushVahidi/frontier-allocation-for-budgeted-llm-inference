...
Frontier is the Class~A accounting diagnostic: a reserved-attempt full-expansion policy with fixed
parameters across cells. It runs two direct-chain attempts, forms a gold-free incumbent by plurality,
then uses the remaining nominal budget for challenger expansion/verification before applying a
fixed gold-free override rule. On preserved validated records, reconstructed successful calls
average $5.38$ of nominal $6$ (Section~\ref{sec:compute}).
...
ties fall back to Frontier). Same extracted answers as FTA; no model calls.
...
\end{itemize}
FTA is the transfer diagnostic for failure-mined commit semantics. Gate rules were developed on
Cohere$\times$GSM8K failure analysis; that cell is development-adjacent
