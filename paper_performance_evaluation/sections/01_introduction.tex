...
\begin{figure}[t]
\centering
\resizebox{\linewidth}{!}{\input{figures/figure1_protocol_schematic}}
...
The deliverable is a protocol-level measurement specification for budgeted closed-API inference
(Figure~\ref{fig:protocol}). It defines what must be counted separately (nominal actions,
successful completions, retries, tokens, latency, and dollars), how post-generation selectors should
be attributed on identical pools, how provider-transfer evidence should be labeled, and how
blocked/non-convergent cells should be reported as performance outcomes. The empirical sections use
that protocol to show four consequences: identical-pool plurality is a required selector control,
failure-mined rules need transfer labels, nominal budgets are not realized resource vectors, and
provenance/blocked cells affect interpretation even when accuracy values are unchanged.
...
The named procedures are controlled objects for exercising the protocol, not promoted inference
algorithms (Section~\ref{sec:method}). Class~A single-generator policies under $B{=}6$ include a
