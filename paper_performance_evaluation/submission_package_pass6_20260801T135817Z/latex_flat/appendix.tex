...
counterfactuals, and compute detail. Historical negative variants, pilot surfaces, and partial
same-model checks are moved to the supplementary material because they are outside the evaluated
protocol.
...
separate from runtime leakage (Section~\ref{sec:limitations}).
...
repair-related field on \textbf{151} of $3{,}394$ Frontier rows. Among Frontier differs,
hypothetical repair application yields 0 rescues and 68 regressions in this reconstruction
(remainder wrong$\leftrightarrow$wrong / neutral).
...
Tier~A: Cohere/Azure/Vertex$\times$GSM8K (strongest draw and scoring provenance under the released
...
A public claim--evidence map links numerical claims to tracked aggregate audit records, source
tables, analysis scripts, and package checksums for the Azure true-SC comparison, held-out selector,
and 15-cell matrix summaries. The public artifacts support aggregate-record verification of the
reported numerical invariants, not identical proprietary regeneration or every historical subset draw
(Section~\ref{sec:setup-repro}). Live reruns would require provider credentials and paid API calls
and are not part of this manuscript.

\section{Supplementary Diagnostics}
\label{app:supp-diagnostics}

The descriptive discovery--selection scatter diagnostic is provided in the supplementary material;
the tested transfer evidence used in the main paper is reported in Section~\ref{sec:provider-analysis}.
