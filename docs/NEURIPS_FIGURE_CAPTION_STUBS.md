...
- **Table 4.** Anti-collapse calibration summary combining performance and concentration/diversity diagnostics; interpret as calibration tradeoff evidence (not independent universal component validation).
...
- **Appendix Figure A4.** Strict-F3 component-ablation deltas on the matched manuscript-facing surface, plotted as accuracy differences (percentage points) relative to Full Strict-F3. The ablation is non-monotonic: removing default anti-collapse improves the original paper surface. The calibration follow-up (Table 4) shows weak anti-collapse \(>\) default, off \(>\) default, strong \(\approx\) default, and conditional \(<\) default, supporting recalibration language rather than universal removal or universal benefit.
