...
def _is_sentinel_only_text(text: str | None) -> bool:
    """True if ``text`` is composed entirely of the ``model_step_missing`` step-trace
    placeholder (see ``experiments/branching.py``'s ``expand()``, which appends this literal
    when a step produced no extractable text) -- one occurrence or many, newline-joined --
    with no other content. Such text is a missing-step marker, never real model output, and
    must not be surfaced as an answer.
    """
    if not text:
        return False
    parts = [p.strip() for p in text.split("\n")]
    parts = [p for p in parts if p]
    return bool(parts) and all(p == _MODEL_STEP_MISSING_SENTINEL for p in parts)


def _contains_sentinel_marker(text: str | None) -> bool:
    """True if the ``model_step_missing`` placeholder appears anywhere in ``text``.

    Matches both whole-line occurrences (even mixed with other step content) and
    substring occurrences after canonicalization collapses newlines (observed on real
    Vertex Gemini MATH-500 full300 row HuggingFaceH4_MATH-500_213). Unlike
    ``_is_sentinel_only_text``, which only catches the degenerate all-sentinel case,
    any sentinel presence makes the text unsafe to surface or mine with
    ``extract_final_answer`` (see docs/CLOUDRIFT_GPQA_REMAINING_PARTIAL_DIAGNOSIS_20260705.md
    and docs/VERTEX_GEMINI_MATH500_SENTINEL_FORENSIC_20260705.md).
    """
    if not text:
        return False
    return _MODEL_STEP_MISSING_SENTINEL in text


def _is_usable_answer_text(text: str | None) -> bool:
    """Gold-free guard: text is non-empty and free of sentinel-only or mixed-sentinel content."""
    if not text:
        return False
    return not _is_sentinel_only_text(text) and not _contains_sentinel_marker(text)
...
def deterministic_extract_answer(
    *,
    ...
    This is a second, independent call site for ``extract_final_answer`` on raw
    reasoning text (the other is inside ``effective_answer_raw_from_node``) -- it must
    carry the same sentinel guard, since ``chosen_node_answer_raw`` here is frequently
    the (correctly ``None``) output of ``effective_answer_raw_from_node`` when that
    function declined a sentinel-contaminated node, and this function would otherwise
    re-derive an unguarded value from the exact same contaminated ``reasoning_text``
    (observed for real CloudRift and Gemini GPQA-MCQ examples; see
    docs/CLOUDRIFT_GPQA_REMAINING_PARTIAL_DIAGNOSIS_20260705.md and
    docs/VERTEX_GEMINI_GPQA_MCQ_RESMOKE_AFTER_BUDGET_FAIRNESS_FIX_20260705.md).
    """
    direct = _to_text(chosen_node_answer_raw)
    reasoning_ok = _is_usable_answer_text(chosen_branch_reasoning_text)
    extracted = _to_text(extract_final_answer(chosen_branch_reasoning_text or "")) if reasoning_ok else None
    if not _is_usable_answer_text(extracted):
        extracted = None
    if _is_usable_answer_text(direct):
