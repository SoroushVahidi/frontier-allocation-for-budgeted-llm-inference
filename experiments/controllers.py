...
from experiments.branching import (
    BranchState,
    is_mcq_question,
    is_strategyqa_question,
    sanitize_mcq_final_answer,
    gpqa_terminal_finalization_fallback_eligible,
)
...
from experiments.frontier_max_support_tiebreak import (
    apply_gated_conservative_answer_group_tiebreak,
...
from experiments.selection_logging_enrichment import build_selection_logging_enrichment
...
class BaseController:
    def __init__(self, generator: BranchGenerator, scorer: BranchScorer, max_actions_per_problem: int) -> None:
        ...
        # Per-example usage ledger. Some controllers (e.g. DirectReserveFrontierGateController
        # and relatives) construct additional, independent generator instances for a
        # challenger-expansion sub-phase via strict_controller_factory(); those generators'
        # usage counters were previously invisible to callers that only reset/read
        # self.generator, silently undercounting realized calls/tokens/retries for any
        # method with such a sub-phase. register_phase_generator() lets a controller declare
        # such a generator so reset_usage_counters()/snapshot_usage_counters() aggregate
        # across every phase, not just the first one. Controllers with no sub-phase generator
        # get a list that stays empty, so behavior for them is unchanged.
        self._phase_generators: list = []

    def register_phase_generator(self, generator: Any) -> None:
        """Register an additional generator instance (e.g. one built for a challenger-expansion
        sub-phase) so its usage counters are included by reset_usage_counters()/
        snapshot_usage_counters(). Call once per sub-phase generator, per example (the list is
        not cleared automatically between examples; callers that reuse a controller across many
        examples must call reset_usage_counters() at the start of each example, which also
        clears this registry -- see reset_usage_counters() below)."""
        if generator is not None:
            self._phase_generators.append(generator)

    def _iter_usage_bases(self) -> list:
        bases = []
        primary_base = getattr(self.generator, "base", None)
        if primary_base is not None and hasattr(primary_base, "reset_usage_counters"):
            bases.append(primary_base)
        for gen in self._phase_generators:
            base = getattr(gen, "base", None)
            if base is not None and hasattr(base, "reset_usage_counters"):
                bases.append(base)
        return bases

    def reset_usage_counters(self) -> None:
        """Reset usage counters on self.generator.base AND every registered phase generator's
        base, then clear the phase-generator registry (a fresh set of sub-phase generators will
        be registered, if any, during the next run() call for the next example)."""
        for base in self._iter_usage_bases():
            base.reset_usage_counters()
        self._phase_generators = []

    def snapshot_usage_counters(self) -> dict[str, int]:
        """Sum usage counters across self.generator.base and every phase generator's base that
        was registered since the last reset_usage_counters() call. For controllers with no
        sub-phase generators this is identical to reading self.generator.base directly."""
        combined = {"input_tokens": 0, "output_tokens": 0, "total_tokens": 0, "retry_attempts": 0, "api_calls": 0}
        primary_base = getattr(self.generator, "base", None)
        bases = []
        if primary_base is not None and hasattr(primary_base, "snapshot_usage_counters"):
            bases.append(primary_base)
        for gen in self._phase_generators:
            base = getattr(gen, "base", None)
            if base is not None and hasattr(base, "snapshot_usage_counters"):
                bases.append(base)
        for base in bases:
            usage = base.snapshot_usage_counters()
            for key in combined:
                combined[key] += int(usage.get(key, 0) or 0)
        return combined

    @staticmethod
    def _maybe_apply_gpqa_terminal_fallback(
        generator: BranchGenerator, question: str, prediction: str | None,
    ) -> tuple[str | None, dict[str, Any]]:
        """Shared one-shot GPQA terminal finalization fallback hook, available identically
        to Frontier/L1/S1/TALE (fairness requirement -- see
        docs/GPQA_TERMINAL_FINALIZATION_FALLBACK_FAIRNESS_20260717.md). Returns
        (possibly-updated prediction, metadata dict to merge into the method's own
        MethodResult.metadata). Always returns terminal_finalization_fallback_used (True/False)
        in the metadata so downstream analysis can measure fallback usage rate even when the
        gate declines to fire.
        """
        # Controllers are handed scripts/run_cohere_real_model_cost_normalized_validation.py's
        # ObservedGenerator wrapper in production, which proxies ONLY
        # init_branch/expand/verify/prune to its own .base attribute -- provider, model,
        # last_response_text, and run_gpqa_terminal_finalization_fallback all live on the
        # real underlying APIBranchGenerator, reachable via .base. This was a real,
        # confirmed-live bug (getattr(generator, "provider", "") silently returned "" for
        # every real run, so the gate always declined even when eligible) caught only via
        # explicit debug instrumentation on a real hard-example call, not by any mock-based
        # unit test -- every existing test constructs a plain, unwrapped stub generator, so
        # none of them exercised the wrapper case. Falls back to the generator itself for
        # plain (non-wrapped) generators, e.g. this file's own unit tests.
        real_generator = getattr(generator, "base", None)
        if real_generator is None:
            real_generator = generator
        provider = str(getattr(real_generator, "provider", "") or "")
        model = str(getattr(real_generator, "model", "") or "")
        eligible = gpqa_terminal_finalization_fallback_eligible(
            prediction=prediction, question=question, provider=provider, model=model,
        )
        if not eligible:
            return prediction, {"terminal_finalization_fallback_used": False}
        fallback_answer, fallback_meta = real_generator.run_gpqa_terminal_finalization_fallback(question)
        fallback_meta = dict(fallback_meta)
        fallback_meta["terminal_finalization_original_prediction"] = prediction
        if fallback_answer:
            # This is the field apply_controller_committed_surfacing_for_evaluation() (in
            # experiments/output_layer_repair.py, called from evaluate_with_diagnostics() in
            # the runner script) actually reads to decide what gets persisted as
            # final_answer_raw/final_answer_canonical -- the runner rebuilds final_nodes from
            # the live branch registry's own predicted_answer attributes, NOT from
            # MethodResult.prediction, so setting prediction alone (without this) is invisible
            # to scoring. This is the same "controller_metadata_final_answer" commit site the
            # sanitize_mcq_final_answer guard already uses elsewhere (5 sites, see
            # docs/MCQ_CONTROLLER_FINAL_ANSWER_GUARD_20260705.md).
            fallback_meta["final_answer"] = fallback_answer
...
class S1BudgetForcingController(BaseController):
    ...
    @staticmethod
    def _strategyqa_question_with_finalization(question: str, *, force_finalize: bool) -> str:
        suffix = (
            "Reason briefly. If you can answer, end explicitly with Final answer: True or Final answer: False. "
            "If you follow structured output, answer should be the JSON boolean true or false."
        )
        if force_finalize:
            suffix = (
                "Finalize now. End explicitly with Final answer: True or Final answer: False. "
                "If you follow structured output, answer should be the JSON boolean true or false."
            )
        return f"{question}\n\n{suffix}"
    ...
    def run(self, question: str, gold_answer: str) -> MethodResult:
        ...
        action_trace: list[dict[str, Any]] = []
        is_strategyqa = is_strategyqa_question(question)
        last_valid_prediction: str | None = None
        ...
        while actions < self.max_actions:
            ...
            if branch.is_done:
                ...
                if need_min_thinking or can_force_continue:
                    ...
                    if is_strategyqa and branch.predicted_answer is not None:
                        last_valid_prediction = branch.predicted_answer
                    branch.is_done = False
                    branch.predicted_answer = None
                    if is_strategyqa:
                        branch.steps.append(
                            f"{self.wait_token} (forced-continue). Finalize with explicit True/False."
                        )
                    else:
            ...
            if is_strategyqa:
                active_question = self._strategyqa_question_with_finalization(
                    question,
                    force_finalize=bool(forced_continue_events > 0 or actions >= self.max_actions - 1),
                )
            self.generator.expand(branch, active_question, gold_answer)
            actions += 1
            expansions += 1
            surviving_trace.append(1)
            if branch.predicted_answer is not None:
                last_valid_prediction = branch.predicted_answer
            ...
            action_trace.append(
                {
                    ...
                    "answer_extraction_source": str(latest.get("expand_answer_extraction_source", "") or ""),
                    "predicted_answer_after_action": (
                        None if branch.predicted_answer is None else str(branch.predicted_answer)
                    ),
                    "forced_continue_events_so_far": int(forced_continue_events),
                }
            )

        prediction = branch.predicted_answer
        prediction_restored_from_prior_final = False
        if is_strategyqa and prediction is None and last_valid_prediction is not None:
            prediction = last_valid_prediction
            prediction_restored_from_prior_final = True
        prediction, gpqa_fallback_meta = self._maybe_apply_gpqa_terminal_fallback(
            self.generator, question, prediction
        )
        ...
        return MethodResult(
            ...
            metadata={
                **gpqa_fallback_meta,
                ...
                "strategyqa_finalization_hint_enabled": is_strategyqa,
                "strategyqa_prediction_restored_from_prior_final": prediction_restored_from_prior_final,
                "strategyqa_last_valid_prediction": last_valid_prediction,
                "action_trace": action_trace,
            },
        )
...
class TALEPromptBudgetingController(BaseController):
    ...
    @staticmethod
    def _strategyqa_budget_text(budget_tokens: int) -> str:
        return (
            f"Use at most {int(budget_tokens)} tokens. Reason briefly and finish with an explicit final "
            "answer: Final answer: True or Final answer: False. If you follow structured output, answer should be the JSON boolean "
            "true or false."
        )

    def _compose_budgeted_question(self, question: str, budget_tokens: int) -> str:
        if is_strategyqa_question(question):
            budget_text = self._strategyqa_budget_text(budget_tokens)
            return f"{question}\n\n{budget_text}"
    ...
    def run(self, question: str, gold_answer: str) -> MethodResult:
        ...
        budget_text = (
            self._strategyqa_budget_text(budget_tokens)
            if is_strategyqa_question(question)
            else self.prompt_template.format(budget=budget_tokens)
        )
        budgeted_question = self._compose_budgeted_question(question, budget_tokens)
        ...
        return MethodResult(
            ...
            metadata={
                **gpqa_fallback_meta,
                ...
                "deviations_from_official": self.deviations_from_official,
                "final_score": self.scorer.score_branch(branch),
                "strategyqa_prompt_adapted": is_strategyqa_question(question),
                "action_trace": action_trace,
            },
        )
...
class DirectReserveGateRerankController(BaseController):
    ...
    @staticmethod
    def _strategyqa_direct_prompt_style(idx: int) -> str:
        if idx % 2 == 0:
            return (
                "Do not restate the question. Return only the JSON object."
            )
        return (
            "Use an independent reasoning angle from earlier attempts. Do not restate the question. "
    ...
    def _run_direct_attempt(self, question: str, gold_answer: str, idx: int, max_actions: int) -> tuple[str | None, int, list[dict[str, Any]]]:
        ...
        if is_strategyqa_question(question):
            prompt_style = self._strategyqa_direct_prompt_style(idx)
            prompt = question if idx == 0 else f"{question}\n\n{prompt_style}"
        else:
            ...
            trace.append(
                {
                    ...
                    "reasoning_text": str(latest.get("reasoning_text", "")),
                    "extracted_answer": str(latest.get("extracted_answer", "") or ""),
                    "expand_answer_extraction_source": str(latest.get("expand_answer_extraction_source", "") or ""),
                    "request_meta": dict(latest.get("request_meta") or {}),
                }
            )
    ...
    def run(self, question: str, gold_answer: str) -> MethodResult:
        ...
        if remaining_budget > 0 and gate_decision == "allow_frontier":
            ...
            self.register_phase_generator(getattr(frontier, "generator", None))
...
class DirectReserveFrontierGateController(DirectReserveGateRerankController):
    ...
    def run(self, question: str, gold_answer: str) -> MethodResult:
        ...
        for i in range(reserve_attempts):
            if direct_actions >= self.max_actions:
                break
            # Reserve at least 1 action for each remaining planned slot so a single
            # non-converging slot (repeatedly emitting action="continue" without ever
            # reaching "final") cannot consume the entire per-example budget and starve
            # later planned direct-reserve candidates of any execution at all -- see
            # docs/DIRECT_RESERVE_BUDGET_FAIRNESS_FIX_20260705.md. A no-op when only one
            # slot is planned (slots_remaining_after_this == 0).
            slots_remaining_after_this = max(0, reserve_attempts - i - 1)
            attempt_budget = min(
                per_attempt_cap,
                self.max_actions - direct_actions - slots_remaining_after_this,
            )
        ...
        final_answer, tiebreak_meta = apply_gated_conservative_answer_group_tiebreak(
            final_answer,
            enabled=bool(getattr(self, "enable_frontier_max_support_tiebreak", False)),
            answer_group_support_counts=dict(combined_group_counts_base),
        ...
        # MCQ/GPQA guard: the numeric side-channel mechanisms above (opcheck, decomp-eq,
        # PAL/code-execution, unit-tracking, outcome-verifier rerank, targeted retry) are
        # all built for GSM8K/MATH-style numeric answers and are not MCQ-aware; any of
        # them can override `final_answer` with a numeric value even for a lettered
        # multiple-choice question. Reject that here so a controller-committed final
        # answer for MCQ is always A/B/C/D or None, never numeric or sentinel-tainted --
        # see docs/MCQ_CONTROLLER_FINAL_ANSWER_GUARD_20260705.md.
        final_answer_committed_raw = final_answer
        final_answer = sanitize_mcq_final_answer(final_answer, question)
        ...
        metadata.update(
            build_selection_logging_enrichment(
                final_answer_raw=final_answer_committed_raw,
                final_answer=final_answer,
                answer_group_support_counts=dict(combined_group_counts),
                selector_candidate_pool=selector_candidate_pool,
                ...
                normalize_answer_fn=_normalize_answer,
            )
        )
        return MethodResult(
            method=self.method_name,
...
class DirectReserveFrontierGateV2Controller(DirectReserveFrontierGateController):
    ...
    def run(self, question: str, gold_answer: str) -> MethodResult:
        ...
        fallback_answer, gpqa_fallback_meta = self._maybe_apply_gpqa_terminal_fallback(
            self.generator, question, base.prediction
        )
        fallback_metadata = dict(base.metadata or {})
        fallback_metadata.update(gpqa_fallback_meta)
        if fallback_answer != base.prediction:
            # Keep direct_reserve_answer consistent so the guard-applied path below (rare,
            # and effectively unreachable when prediction was None -- see fairness_audit.md)
            # cannot silently revert to the pre-fallback None if it ever does trigger.
            fallback_metadata["direct_reserve_answer"] = fallback_answer
        base = MethodResult(
            method=base.method,
            prediction=fallback_answer,
            is_correct=self._answers_match(fallback_answer, gold_answer),
        ...
        final_answer = sanitize_mcq_final_answer(metadata.get("direct_reserve_answer"), question)
            ...
            md.update(
                {
                    ...
                    "frontier_candidate_family_count": 0,
                    "reserve_used": True,
                    "final_answer": sanitize_mcq_final_answer(protected_answer, question),
                }
            )
            return MethodResult(
                method=self.method_name,
...
class L1LengthControlController(BaseController):
    ...
    @staticmethod
    def _strategyqa_prompt_style() -> str:
        return (
            "Reason briefly and end explicitly with Final answer: True or Final answer: False. "
            "If you follow structured output, answer should be the JSON boolean true or false."
        )
    ...
    def _compose_budgeted_question(self, question: str) -> str:
        ...
        if is_strategyqa_question(question):
            return f"{question}\n\n{self._strategyqa_prompt_style()} {length_instruction}"
        prompt_style = self.prompt_style
        # GPQA-style 4-choice MCQ questions (marker added by experiments/hf_datasets.py)
        # must not be told to output a \boxed{} numeric answer -- that directly conflicts
        # with the letter-choice instruction APIBranchGenerator._expand_prompt already
        # gives for these questions. Purely additive: only fires for the MCQ marker, so
        # GSM8K/MATH-500 L1 prompts are unchanged.
        if "Answer with a single letter (A, B, C, or D)." in (question or "") and "\\boxed" in prompt_style:
            prompt_style = "Let's think step by step and state the final choice as a single letter (A, B, C, or D)."
        return f"{question}\n\n{prompt_style} {length_instruction}"
