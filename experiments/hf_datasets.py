...
HF_DATASET_SPECS: dict[str, HFDatasetSpec] = {
    ...
    "ChilleD/StrategyQA": HFDatasetSpec(
        key="ChilleD/StrategyQA",
        ...
        answer_fields=("answer",),
        provenance_note=(
            "Binary (True/False) multi-hop commonsense benchmark. test=687, train=1603 rows "
            "(confirmed via datasets.get_dataset_split_names on 2026-07-16). Runtime prompts "
            "must use ONLY the `question` field -- `facts` (supporting evidence) and "
            "`description`/`term` (background context) are benchmark annotation/context "
            "fields, not part of the intended cold-start input, and must never be exposed to "
            "the model at generation time (see scripts/build_strategyqa_problems_file.py)."
        ),
    ),
...
def _format_standard_example(
    *,
    ...
    if spec.key == "allenai/drop":
        ...
        # GPQA Diamond ships no "choices" column -- the 4-choice MCQ options must be
        # built from Correct Answer + Incorrect Answer 1/2/3. Prior to this fix,
        # rec["choices"] was never populated (the lookup above always missed) and the
        # model was sent the bare question stem, i.e. evaluated as open-ended
        # free-response rather than the standard 4-choice MCQ task GPQA Diamond is
        # designed for. See docs/PHASE1A_FAILURE_CASE_CORPUS_20260704T161320Z.md
        # (gpqa_schema_blocker_report.md) for the audit that found this. The shuffle
        # below mirrors the already-correct construction used by the simulated
        # pipeline in scripts/run_non_math_dataset_expansion.py::_load_gpqa_examples.
        correct = _safe_preview(row.get("Correct Answer", answer)).strip()
        wrong = [
            _safe_preview(row.get("Incorrect Answer 1")).strip(),
            _safe_preview(row.get("Incorrect Answer 2")).strip(),
            _safe_preview(row.get("Incorrect Answer 3")).strip(),
        ]
        options = [correct] + [w for w in wrong if w]
        # Deterministic, gold-position-blind shuffle: seeded from idx/split/config,
        # not from which slot the correct answer occupies.
        rng = random.Random(f"gpqa_option_shuffle::{split}::{config}::{idx}")
        rng.shuffle(options)
        labels = ["A", "B", "C", "D"]
        if len(options) < 4:
            options += ["(missing option)"] * (4 - len(options))
        option_block = "\n".join(f"{labels[j]}. {options[j]}" for j in range(4))
        gold_letter = labels[options.index(correct)] if correct in options else ""
        if question:
            rec["question"] = (
                f"{question}\n\n{option_block}\n\n"
                "Answer with a single letter (A, B, C, or D)."
            )
        rec["choices"] = option_block
        if gold_letter:
            rec["answer"] = gold_letter
        rec["gold_answer_text"] = correct
