...
def test_pattern_discovery_dry_run_limit_plans_one_batch_per_provider(tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        labeler,
        "_load_cohere_client",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("Cohere client instantiated in dry run")),
    )
...
def test_provider_config_check_reports_openai_without_clients(tmp_path: Path, monkeypatch) -> None:
    ...
    target15 = tmp_path / "target_15.jsonl"
    structural = tmp_path / "candidate_feature_rows.csv"
    out_dir = tmp_path / "out_check_openai"
    ...
    monkeypatch.setattr(
        labeler,
        "_load_openai_client",
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("OpenAI client instantiated in config check")),
    )
    monkeypatch.setattr(
        labeler,
        "_load_cohere_client",
    ...
    manifest = json.loads((out_dir / "manifest.json").read_text(encoding="utf-8"))
    assert manifest["check_provider_config"] is True
    assert manifest["provider_models"]["openai"] == "openai-x"
    assert manifest["provider_models"]["cohere"] == labeler.DEFAULT_MODELS["cohere"]
