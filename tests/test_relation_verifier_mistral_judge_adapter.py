"""Tests for the Mistral RelationReady judge adapter."""
...
SCRIPT = 'scripts/run_relation_verifier_mistral_judge_adapter.py'
...
def _mistral_payload_row(row_id='rrseed_test_001', prompt=None, model='mistral-small-latest'):
    return {
        'row_id': row_id,
        'provider': 'mistral',
        'model': model,
        ...
        'payload': {
            'model': model,
            'messages': [{'role': 'user', 'content': prompt or _CLEAN_PROMPT}],
            'temperature': 0.0,
...
def test_dry_run_writes_manifest_and_report(tmp_path):
    ...
    write_jsonl(payloads_path, [_mistral_payload_row()])
    ...
    assert (output_dir / 'mistral_dry_run_manifest.jsonl').exists()
    assert (output_dir / 'mistral_adapter_report.md').exists()


def test_dry_run_manifest_has_correct_row_id(tmp_path):
    payloads_path = tmp_path / 'payloads.jsonl'
    write_jsonl(payloads_path, [_mistral_payload_row('rrseed_abc')])
    output_dir = tmp_path / 'output'
    ...
    manifest = read_jsonl(output_dir / 'mistral_dry_run_manifest.jsonl')
...
def test_dry_run_filters_non_mistral_rows(tmp_path):
    cohere_row = {**_mistral_payload_row(), 'provider': 'cohere'}
    payloads_path = tmp_path / 'payloads.jsonl'
    write_jsonl(payloads_path, [_mistral_payload_row(), cohere_row])
    output_dir = tmp_path / 'output'
...
def test_dry_run_max_rows(tmp_path):
    rows = [_mistral_payload_row(f'rrseed_{i:03d}') for i in range(5)]
    payloads_path = tmp_path / 'payloads.jsonl'
...
def test_dry_run_report_confirms_no_api_calls(tmp_path):
    ...
    report = (output_dir / 'mistral_adapter_report.md').read_text()
...
def test_dry_run_detects_missing_payload(tmp_path):
    bad_row = {**_mistral_payload_row(), 'payload': None}
    payloads_path = tmp_path / 'payloads.jsonl'
...
def test_dry_run_detects_wrong_provider(tmp_path):
    bad_row = {**_mistral_payload_row(), 'provider': 'openai'}
    payloads_path = tmp_path / 'payloads.jsonl'
    write_jsonl(payloads_path, [bad_row])
    output_dir = tmp_path / 'output'
...
def test_dry_run_detects_leakage_in_prompt(tmp_path):
    ...
    payloads_path = tmp_path / 'payloads.jsonl'
    write_jsonl(payloads_path, [_mistral_payload_row(prompt=bad_prompt)])
    output_dir = tmp_path / 'output'
...
def test_api_mode_fails_with_missing_api_key(tmp_path):
    ...
    # regardless of whether MISTRAL_API_KEY happens to be set in CI.
    result = run_script(['--payloads-jsonl', str(payloads_path),
                         '--output-dir', str(output_dir),
                         '--mode', 'api',
                         '--allow-api',
                         '--api-key-env', 'MISTRAL_API_KEY_DEFINITELY_NOT_SET_XYZ'],
                        expect_success=False)
...
def test_mock_api_normalizes_valid_response(tmp_path):
    ...
    assert row['judge_name'] == 'mistral:mistral-small-latest'


def test_mock_api_judge_name_includes_model(tmp_path):
    payloads_path = tmp_path / 'payloads.jsonl'
    mock_path = tmp_path / 'mock.jsonl'
    write_jsonl(payloads_path, [_mistral_payload_row(model='mistral-large-latest')])
    ...
    normalized = read_jsonl(output_dir / 'normalized_judge_responses.jsonl')
    assert normalized[0]['judge_name'] == 'mistral:mistral-large-latest'
...
def test_mock_api_unknown_row_id_excluded(tmp_path):
    payloads_path = tmp_path / 'payloads.jsonl'
    mock_path = tmp_path / 'mock.jsonl'
    write_jsonl(payloads_path, [_mistral_payload_row('rrseed_real')])
...
def test_mock_api_multiple_valid_responses(tmp_path):
    rows = [_mistral_payload_row(f'rrseed_{i:03d}') for i in range(3)]
...
def test_dry_run_start_index_slices_rows(tmp_path):
    rows = [_mistral_payload_row(f'rrseed_{i:03d}') for i in range(10)]
    payloads_path = tmp_path / 'payloads.jsonl'
    write_jsonl(payloads_path, rows)
    output_dir = tmp_path / 'output'
